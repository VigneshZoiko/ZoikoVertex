...
export class PromptTestService {
  ...
  static async listRuns(versionId: string, options?: { excludeAdversarial?: boolean }) {
    const { data, error } = await supabaseAdmin
    ...
    if (error) throw error;
    let results = data || [];
    if (options?.excludeAdversarial) {
      results = results.filter((r: any) => !r.run_metadata?.adversarial);
    }
    return results;
  }

  static async listAdversarialRuns(versionId: string) {
    const { data, error } = await supabaseAdmin
      .from('prompt_test_runs')
      .select('*')
      .eq('prompt_version_id', versionId)
      .order('created_at', { ascending: false });
    if (error) throw error;
    return (data || []).filter((r: any) => r.run_metadata?.adversarial === true);
  }
  ...
  static async createAdversarialRun(input: {
    prompt_version_id: string;
    suite_id: string;
    pass_fail: string;
    score_summary: Record<string, unknown>;
    ...
    const { data, error } = await supabaseAdmin
      ...
      .insert({
        ...
        score_summary: input.score_summary,
        run_metadata: { ...input.run_metadata, adversarial: true },
