...
export async function getWorkflowAnalytics(workspaceId: string) {
  ...
  const [activeTemplates, totalTemplates, activeRuns, blockedRuns, completedRuns, failedRuns, slaBreached, slaTotal, approvalRecords, pendingApprovals, unhealthyDeps] = results.map((r) => r.count || 0);
