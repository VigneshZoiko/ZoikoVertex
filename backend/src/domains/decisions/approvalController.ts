...
const ROLE_QUEUE_STATUSES: Record<string, string[]> = {
  ...
  ADMIN:             ['PENDING_REVIEW','PENDING_MANAGER','PENDING_VALIDATION','PENDING_AUTHORIZATION','PENDING_GOVERNANCE','PENDING_ADMIN'],
  WORKSPACE_OWNER:   ['PENDING_REVIEW','PENDING_MANAGER','PENDING_VALIDATION','PENDING_AUTHORIZATION','PENDING_GOVERNANCE','PENDING_ADMIN'],
};
...
const RISK_PATHS: Record<string, string[]> = {
  ...
  HIGH:       ['PENDING_MANAGER', 'PENDING_VALIDATION', 'PENDING_AUTHORIZATION', 'APPROVED'],
  RESTRICTED: ['PENDING_MANAGER', 'PENDING_VALIDATION', 'PENDING_AUTHORIZATION', 'PENDING_GOVERNANCE', 'APPROVED'],
};
...
const ActionSchema = z.object({
  action: z.enum(['approve', 'reject', 'return_revision', 'block', 'escalate', 'validate', 'authorize']),
...
const SubmitForReviewSchema = z.object({
  ...
  risk_override: z.enum(['LOW','STANDARD','ELEVATED','HIGH','RESTRICTED']).optional(),
});

export const submitForReview = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { content, platform, target_account_ids, media_urls, risk_override } = SubmitForReviewSchema.parse(req.body);
...
export const getApprovalQueue = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const userId = req.user?.id;
    if (!userId) return res.status(401).json({ error: 'Unauthorized' });
...
export const getApprovalStats = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    for (const item of all || []) {
      const s = item.status;
      if (['PENDING_REVIEW', 'PENDING_MANAGER'].includes(s)) counts.pending_review++;
      else if (s === 'PENDING_VALIDATION') counts.pending_validation++;
      else if (['PENDING_AUTHORIZATION', 'PENDING_ADMIN'].includes(s)) counts.pending_authorization++;
    ...
    res.json({ success: true, data: { counts, recent: recent || [], approval_rate } });
  } catch (error) {
    next(error);
  }
};
...
export const takeApprovalAction = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const { data: intent, error: fetchErr } = await supabaseAdmin
      ...
      .eq('id', id)
      .single();
    if (fetchErr || !intent) return res.status(404).json({ error: 'Intent not found' });
    ...
    switch (action) {
      ...
      case 'return_revision': nextStatus = 'RETURNED'; break;
      case 'block':          nextStatus = 'GOVERNANCE_BLOCKED'; break;
      case 'escalate':       nextStatus = 'PENDING_GOVERNANCE'; break;
      case 'approve':
      case 'validate':
      case 'authorize': {
    ...
    if (nextStatus === 'APPROVED') {
      const wsId = Array.isArray(intent.workspace_id) ? intent.workspace_id[0] : String(intent.workspace_id);
      ...
      if (!decisionResult.governance_cleared) {
        await supabaseAdmin.from('publish_intents')
          .update({ status: 'GOVERNANCE_BLOCKED', feedback: `Blocked: ${decisionResult.decision_class}`, decision_id: decisionResult.decision_id })
          .eq('id', id);
        return res.status(403).json({ error: 'Governance blocked', decision_class: decisionResult.decision_class });
      }

      await supabaseAdmin.from('publish_intents')
        .update({ status: nextStatus, feedback: feedback || null, decision_id: decisionResult.decision_id })
        .eq('id', id);

      internalEventBus.emit('execution.requested', { intentId: id, orgId: intent.workspace_id });
    } else {
      await supabaseAdmin.from('publish_intents')
