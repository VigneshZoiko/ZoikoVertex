...
describe('Frontend API client endpoint alignment', () => {
  let apiContent = '';
  beforeAll(() => {
    apiContent = readFileSync(
      resolve(__dirname, '../../../../frontend/src/features/agent-operations/api.ts'), 'utf8');
  });

  it('frontend api.ts calls match backend registered routes', () => {
    const frontendEndpoints = [
      ...
      '/api/v1/operations/evidence/',
    ];
    for (const ep of frontendEndpoints) {
      expect(src).toContain(ep);
      expect(apiContent).toContain(ep);
    }
  });

  it('frontend maps hold to /hold and release_hold to /release-hold', () => {
    expect(apiContent).toContain('hold: "hold"');
    expect(apiContent).toContain('release_hold: "release-hold"');
  });
});

describe('Frontend hold/release-hold visibility from action_gates', () => {
  let typesContent = '';
  let pageContent = '';
  ...
  beforeAll(() => {
    typesContent = readFileSync(
      resolve(__dirname, '../../../../frontend/src/features/agent-operations/types.ts'), 'utf8');
    pageContent = readFileSync(
      resolve(__dirname, '../../../../frontend/src/features/agent-operations/AgentOperationsPage.tsx'), 'utf8');
    permissionsContent = readFileSync(
      resolve(__dirname, '../../../../frontend/src/features/agent-operations/permissions.ts'), 'utf8');
    feApiContent = readFileSync(
      resolve(__dirname, '../../../../frontend/src/features/agent-operations/api.ts'), 'utf8');
  });

  it('has hold and release_hold in the RuntimeAction type', () => {
    expect(typesContent).toContain('"hold"');
    expect(typesContent).toContain('"release_hold"');
  });

  it('has hold and release_hold ActionButton entries in the page', () => {
    expect(pageContent).toContain('ActionButton action="hold"');
    expect(pageContent).toContain('ActionButton action="release_hold"');
  });

  it('has hold and release_hold modal descriptions', () => {
    expect(pageContent).toContain('action === "hold"');
    expect(pageContent).toContain('action === "release_hold"');
    expect(pageContent).toContain('lighter-touch intervention');
    expect(pageContent).toContain('resumes a previously held run');
  });

  it('has human_review_time and incident_closure_time in OperationsAnalytics type', () => {
    expect(typesContent).toContain('human_review_time');
    expect(typesContent).toContain('incident_closure_time');
  });

  it('renders human_review_time and incident_closure_time in analytics section', () => {
    expect(pageContent).toContain('human_review_time?.value');
    expect(pageContent).toContain('incident_closure_time?.value');
  });

  it('uses server action_gates as source of truth', () => {
    expect(permissionsContent).toContain('if (run.action_gates');
    expect(permissionsContent).toContain('Prefer server-computed action_gates');
  });

  it('has hold and release_hold in performRunAction endpoint map', () => {
    expect(feApiContent).toContain('hold: "hold"');
    expect(feApiContent).toContain('release_hold: "release-hold"');
  });

  it('has exportAnalyticsCSV and exportOutputSnapshot in frontend api', () => {
    expect(feApiContent).toContain('exportAnalyticsCSV');
    expect(feApiContent).toContain('exportOutputSnapshot');
  });

  it('has brand filter in OperationsFilters', () => {
    const typesContent = readFileSync(
      resolve(__dirname, '../../../../frontend/src/features/agent-operations/types.ts'), 'utf8');
    expect(typesContent).toContain('brand_id');
    expect(typesContent).toContain('brand_name');
  });

  it('has brand selector in AgentOperationsPage filter bar', () => {
    const pageContent = readFileSync(
      resolve(__dirname, '../../../../frontend/src/features/agent-operations/AgentOperationsPage.tsx'), 'utf8');
    expect(pageContent).toContain('BRAND_OPTIONS');
    expect(pageContent).toContain('brand_name');
  });

  it('passes brand filters to all API calls in useAgentOperations', () => {
    const hookContent = readFileSync(
      resolve(__dirname, '../../../../frontend/src/features/agent-operations/useAgentOperations.ts'), 'utf8');
    expect(hookContent).toContain('getStats(filters)');
    expect(hookContent).toContain('listQueues(undefined, filters)');
    expect(hookContent).toContain('listIncidents(filters)');
    expect(hookContent).toContain('getAnalytics(filters)');
  });

  it('has CSV export button and output snapshot export in the page', () => {
    const pageContent = readFileSync(
      resolve(__dirname, '../../../../frontend/src/features/agent-operations/AgentOperationsPage.tsx'), 'utf8');
    expect(pageContent).toContain('export_csv');
    expect(pageContent).toContain('export_output_snapshot');
  });
});
