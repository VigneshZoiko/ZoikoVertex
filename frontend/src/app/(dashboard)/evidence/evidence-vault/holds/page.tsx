    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => setShowApply(true)} className="flex items-center gap-1.5 px-3 py-1.5 bg-red-600 text-xs text-foreground rounded-lg hover:bg-red-700">
          ...
          <tbody>
            {holds.map(hold => (
              <tr key={hold.id} className="border-b border-[#1a1a1a] hover:bg-[#1a1a1a]">
                ...
                <td className="p-3 text-[#aaa] max-w-[200px] truncate">{hold.reason}</td>
                <td className="p-3 text-[#888] font-mono text-[11px]">{hold.requester_id.substring(0, 8)}</td>
                <td className="p-3 text-[#666] text-[11px]">{fmt(hold.effective_date)}</td>
                <td className="p-3">
                  {hold.released
                    ? <span className="text-green-400 flex items-center gap-1"><Unlock className="w-3 h-3" /> Released</span>
                    : <span className="text-red-400 flex items-center gap-1"><Lock className="w-3 h-3" /> Active</span>
                  }
                </td>
              </tr>
            ))}
            {holds.length === 0 && !loading && <tr><td colSpan={8} className="p-8 text-center text-[#555]">No legal holds applied.</td></tr>}
          </tbody>
        </table>
      </div>
      <div className="text-xs text-[#555] mt-2">{total} total holds</div>
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center" onClick={onClose}>
      <div className="bg-[#151515] border border-[#333] rounded-xl p-6 w-full max-w-lg mx-4" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-3 max-h-[60vh] overflow-y-auto">
          <div>
            <label className="text-xs text-[#888] mb-1 block">Scope Type</label>
          ...
          <div>
            ...
            <input value={scopeId} onChange={e => setScopeId(e.target.value)}
              placeholder="UUID of the item/collection/package" className="w-full bg-[#1a1a1a] border border-[#333] rounded-lg px-3 py-2 text-xs text-foreground" />
          </div>
          <div>
            ...
            <input value={matterRef} onChange={e => setMatterRef(e.target.value)}
              placeholder="e.g. MATTER-2026-001" className="w-full bg-[#1a1a1a] border border-[#333] rounded-lg px-3 py-2 text-xs text-foreground" />
          </div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button onClick={handleSave} disabled={saving}
              className="px-4 py-2 text-xs text-foreground bg-red-600 rounded-lg hover:bg-red-700 disabled:opacity-50">
