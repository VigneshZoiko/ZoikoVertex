    ...
    <div className="p-8 max-w-5xl mx-auto space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-zinc-400 mt-1 text-sm">Monitor authentication events, audit activity, and workspace access.</p>
        </div>
        <button
          onClick={load}
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="p-5 bg-zinc-900/50 border border-zinc-800 rounded-2xl">
          ...
          <p className="text-xs text-zinc-500 font-medium uppercase tracking-wider">Team Members</p>
          <p className="text-3xl font-black text-white mt-1">{members.length}</p>
        </div>
        <div className="p-5 bg-zinc-900/50 border border-zinc-800 rounded-2xl">
          <FileSearch className="w-5 h-5 text-emerald-400 mb-3" />
          <p className="text-xs text-zinc-500 font-medium uppercase tracking-wider">Recent Events</p>
          <p className="text-3xl font-black text-white mt-1">{auditEvents.length}</p>
        </div>
        <div className="p-5 bg-zinc-900/50 border border-zinc-800 rounded-2xl">
          <ShieldCheck className={`w-5 h-5 mb-3 ${highCount > 0 ? "text-rose-400" : "text-emerald-400"}`} />
          <p className="text-xs text-zinc-500 font-medium uppercase tracking-wider">High Severity</p>
          <p className={`text-3xl font-black mt-1 ${highCount > 0 ? "text-rose-400" : "text-white"}`}>{highCount}</p>
        </div>
      </div>
      ...
      <div className="bg-zinc-900/50 border border-zinc-800 rounded-3xl overflow-hidden">
        <div className="px-6 py-5 border-b border-zinc-800 flex items-center justify-between">
          <h2 className="font-bold text-white">Team Members &amp; Access</h2>
        ...
        <div className="divide-y divide-zinc-800/50">
          {members.length === 0 ? (
            <p className="px-6 py-8 text-center text-zinc-600 text-sm">No members found</p>
          ) : members.map((m, i) => (
            <div key={m.id || i} className="px-6 py-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-white">{m.full_name || m.email || m.user_id}</p>
                  <p className="text-xs text-zinc-500 mt-0.5">{m.email || m.user_id}</p>
                </div>
              </div>
      ...
      <div className="bg-zinc-900/50 border border-zinc-800 rounded-3xl overflow-hidden">
        <div className="px-6 py-5 border-b border-zinc-800 flex items-center justify-between">
          <h2 className="font-bold text-white">Recent Audit Events</h2>
        ...
        <div className="divide-y divide-zinc-800/50">
          {auditEvents.length === 0 ? (
            <p className="px-6 py-8 text-center text-zinc-600 text-sm">No recent audit events</p>
            ...
            return (
              <div key={event.id || i} className="px-6 py-3 flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-zinc-200 font-medium truncate">{event.action}</p>
