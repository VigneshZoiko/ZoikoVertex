  ...
  const handleAddAccount = async (platformId: string) => {
    ...
    try {
      ...
      if (platformId === 'facebook' || platformId === 'instagram') {
        ...
        window.location.assign(`https://www.facebook.com/dialog/oauth?client_id=${appId}&redirect_uri=${redirectUri}&scope=${scopeString}&state=${state}&response_type=code`);
        ...
        window.location.assign(`https://www.pinterest.com/oauth/?client_id=${clientId}&redirect_uri=${redirectUri}&response_type=code&scope=${scope}&state=${state}`);
        ...
        window.location.assign(`https://www.threads.net/oauth/authorize?client_id=${appId}&redirect_uri=${redirectUri}&scope=${scope}&state=${state}&response_type=code`);
      } else {
              ...
              <div className="grid grid-cols-2 gap-3 mb-8">
                {PLATFORMS.map(platform => (
                  <button
                    ...
                    className="flex items-center gap-3 p-4 rounded-2xl bg-[var(--card)]/30 border border-[var(--border)] hover:border-zinc-500 hover:bg-[var(--surface-hover)]/50 transition-all text-left disabled:opacity-50 group"
                  >
                    <div 
