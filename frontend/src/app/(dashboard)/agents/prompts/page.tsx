        ...
        <div className="p-6 border-b border-border space-y-4 sticky top-0 bg-card z-10">
          ...
          <div className="flex flex-wrap gap-1">
            {PRIMARY_DRAWER_TABS.map((t) => (
              <button key={t.id} onClick={() => setDrawerTab(t.id)} className={`px-3 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${drawerTab === t.id ? "bg-indigo-500/20 text-indigo-400 border border-indigo-500/30" : "text-foreground-muted hover:text-white"}`}>
                {t.label}
              </button>
            ))}
          </div>
        ...
        <div className="relative z-10 flex flex-col xl:flex-row items-start xl:items-center justify-between gap-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="px-6 py-3 bg-surface border border-border hover:border-slate-600 text-foreground hover:text-white rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all flex items-center gap-2"
            >
