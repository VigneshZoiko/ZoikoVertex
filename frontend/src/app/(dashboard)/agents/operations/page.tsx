...
interface Agent {
  ...
  status: string; autonomy_level: string;
...
const LEVEL_COLORS: Record<string, string> = {
  L0: "text-[#555]", L1: "text-zinc-400", L2: "text-blue-400",
  L3: "text-amber-400", L4: "text-emerald-400", L5: "text-teal-400", L6: "text-indigo-400",
};

const LEVEL_NAMES: Record<string, string> = {
  L0: "Disabled", L1: "Assistive", L2: "Creative", L3: "Guided",
  L4: "Validated", L5: "Conditional", L6: "Enterprise",
};
...
export default function AgentOperationsPage() {
  ...
  const riskAgents = agents.filter(a => {
    const t = Math.round((a.trust_score || 0) * 100);
    return t < 70 || ["SUPERVISED","SUSPENDED","DEAUTHORIZED","RESTRICTED","MONITORED"].includes(a.status);
  });
    ...
    <div className="max-w-5xl mx-auto pb-16 px-4">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-[#888] text-sm">Live supervision of all agents — status, autonomy, trust scores, and incidents</p>
        </div>
            ...
            {["L0","L1","L2","L3","L4","L5","L6"].map(lvl => {
              ...
              return (
                <div key={lvl} className="flex flex-col items-center gap-1 flex-1">
                  ...
                  <span className={`text-[9px] font-bold ${LEVEL_COLORS[lvl]}`}>{lvl}</span>
            ...
            {riskAgents.length === 0 ? (
              ...
              <div className="space-y-2">
                {riskAgents.map(agent => {
                  ...
                  const scfg = STATUS_CONFIG[agent.status] ?? STATUS_CONFIG.DRAFT;
                  return (
                    <div key={agent.id} className="flex items-center gap-3 p-3 bg-white/2 border border-[var(--border)]/50 rounded-xl">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className={`text-[10px] ${scfg.color}`}>{scfg.label} · Trust {trustPct}%</p>
                      </div>
                      <span className={`text-[10px] font-bold ${LEVEL_COLORS[agent.autonomy_level] ?? "text-[#555]"}`}>
            ...
            {agents.length === 0 ? (
              ...
              <div className="space-y-1.5">
                {agents.map(agent => {
                  const trustPct = Math.round((agent.trust_score || 0) * 100);
                  const scfg = STATUS_CONFIG[agent.status] ?? STATUS_CONFIG.DRAFT;
                  const lvlName = LEVEL_NAMES[agent.autonomy_level] ?? agent.autonomy_level;
                  const lvlColor = LEVEL_COLORS[agent.autonomy_level] ?? "text-[#555]";
                  return (
                    <div key={agent.id} className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-white/3 transition-colors">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <div className="w-16 h-1 bg-white/5 rounded-full overflow-hidden">
                          <div className={`h-full rounded-full ${trustPct >= 80 ? "bg-emerald-500" : trustPct >= 60 ? "bg-amber-500" : "bg-rose-500"}`} style={{ width: `${trustPct}%` }} />
                        </div>
                        <span className={`text-[10px] font-bold ${lvlColor}`}>{agent.autonomy_level}</span>
