...
export default function SuperAdminPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError("Failed to create organization. Verify Workspace Owner privileges.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-xs font-black uppercase tracking-[0.3em] text-indigo-500">Workspace Owner Control Plane</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-zinc-900/40 border border-zinc-800/50 rounded-3xl p-8">
            <h3 className="text-sm font-black text-zinc-500 uppercase tracking-widest mb-6">Approval Queue</h3>
            <div className="space-y-4">
              {fetchingPending ? (
                ...
                pendingOrgs.map(org => (
                  <div key={org.id} className="p-4 bg-zinc-950 border border-zinc-800 rounded-2xl space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[10px] font-black bg-indigo-500/10 text-indigo-400 px-2 py-0.5 rounded uppercase tracking-widest border border-indigo-500/20">Pending</span>
                    </div>
                    <div className="flex items-center gap-2">
                       <button 
                         onClick={() => handleApprove(org.id)}
                         disabled={loading}
                         className="flex-1 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-[11px] font-bold rounded-lg transition-all disabled:opacity-50"
                       >
                         {loading ? "Processing..." : "Approve"}
                       </button>
                       <button 
                         className="px-3 py-2 bg-zinc-900 border border-zinc-800 text-zinc-500 hover:text-rose-400 transition-colors rounded-lg"
                         title="Deny Request"
                       >
                ...
                <div className="flex flex-col items-center justify-center py-10 text-center">
                  ...
                  <p className="text-zinc-600 text-xs font-medium italic">No pending requests.</p>
                </div>
              )}
            </div>
          </div>
