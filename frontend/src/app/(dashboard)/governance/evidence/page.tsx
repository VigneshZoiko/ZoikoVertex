      ...
      <div className="flex justify-between items-start mb-8">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h1 className="text-2xl font-black text-gray-900 dark:text-white">Evidence Vault</h1>
              <p className="text-xs text-[#666]">Sovereign Immutable Audit Chain · Tier-0 Safety Layer</p>
            </div>
          </div>
        </div>
        <div className="flex gap-3">
          <button onClick={fetchAll} className="p-2.5 bg-[#1a1a1a] hover:bg-[#222] border border-[#333] rounded-lg text-gray-900 dark:text-white transition-colors">
            <RefreshCw className="w-4 h-4" />
          </button>
          <button
            onClick={() => setShowPackBuilder(!showPackBuilder)}
            className="flex items-center gap-2 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-gray-900 dark:text-white text-xs font-bold rounded-lg transition-colors"
          >
            ...
            <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-xl p-5 flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="font-mono text-[10px] text-[#888] space-y-1">
                  <p>Pack ID: <span className="text-gray-900 dark:text-white">{buildResult.id}</span></p>
                  <p>Artifacts: <span className="text-gray-900 dark:text-white">{buildResult.artifact_count}</span></p>
                  <p>Hash: <span className="text-blue-400 break-all">{buildResult.export_hash}</span></p>
                </div>
                <div className="flex gap-3 mt-3">
                  <button onClick={() => downloadPack(buildResult.id)} className="flex items-center gap-2 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-gray-900 dark:text-white text-xs font-bold rounded-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
              <div>
                ...
                <select value={packPurpose} onChange={e => setPackPurpose(e.target.value)}
                  className="w-full bg-black border border-[#333] focus:border-blue-500/50 rounded-lg px-3 py-2 text-xs text-gray-900 dark:text-white focus:outline-none">
              ...
              <div>
                ...
                <input value={packScope} onChange={e => setPackScope(e.target.value)}
                  ...
                  className="w-full bg-black border border-[#333] focus:border-blue-500/50 rounded-lg px-3 py-2 text-xs text-gray-900 dark:text-white placeholder-[#555] focus:outline-none" />
              </div>
              <div>
              ...
              <button onClick={buildPack} disabled={building || !packScope.trim()}
                className="px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-gray-900 dark:text-white text-xs font-black rounded-lg transition-colors flex items-center justify-center gap-2">
      ...
      <div className="flex gap-1 bg-[#111] border border-[#222] rounded-xl p-1 mb-6 w-fit">
        {(["artifacts", "packs", "holds"] as const).map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            className={`px-4 py-2 rounded-lg text-xs font-bold capitalize transition-all ${activeTab === tab ? "bg-[#222] text-gray-900 dark:text-white shadow" : "text-[#666] hover:text-white"}`}>
          ...
          <div className="p-4 border-b border-[#222] flex gap-4 items-center">
            <div className="relative flex-1 max-w-sm">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)}
                ...
                className="w-full bg-black border border-[#2d2d2d] focus:border-[#555] rounded-lg pl-9 pr-4 py-1.5 text-xs text-gray-900 dark:text-white placeholder-[#555] focus:outline-none" />
            </div>
              ...
              <tbody>
                {packs.length === 0 ? (
                  ...
                  <tr key={pack.id} className="border-b border-[#1a1a1a] hover:bg-[#141414] transition-colors">
                    ...
                    <td className="px-4 py-3 text-gray-900 dark:text-white font-bold">{pack.artifact_count}</td>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={holdObjectId} onChange={e => setHoldObjectId(e.target.value)}
                  ...
                  className="w-full bg-black border border-[#333] focus:border-purple-500/50 rounded-lg px-3 py-2 text-xs text-gray-900 dark:text-white placeholder-[#555] focus:outline-none" />
              </div>
              <div>
              ...
              <div>
                ...
                <textarea value={holdReason} onChange={e => setHoldReason(e.target.value)}
                  ...
                  className="w-full h-20 bg-black border border-[#333] focus:border-purple-500/50 rounded-lg px-3 py-2 text-xs text-gray-900 dark:text-white placeholder-[#555] resize-none focus:outline-none" />
              </div>
              <button onClick={applyHold} disabled={applyingHold || !holdObjectId || !holdMatter || holdReason.length < 10}
                className="w-full py-2.5 bg-purple-600 hover:bg-purple-700 disabled:opacity-50 text-gray-900 dark:text-white text-xs font-black rounded-lg transition-colors flex items-center justify-center gap-2">
