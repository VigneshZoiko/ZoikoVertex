...
export default function ApprovalsPage() {
  ...
  return (
    <div className="flex flex-col bg-[#0e0e0e] md:h-[calc(100vh-3.5rem)]">
      {/* ─── Alert Strip ─────────────────────────────────────────────────── */}
      ...
      {/* ─── Header ──────────────────────────────────────────────────────── */}
      <div className="flex items-center justify-between gap-2 flex-wrap px-4 py-3 border-b border-[#2d2d2d] shrink-0">
      ...
      {/* ─── Search + Filter Bar ────────────────────────────────────────── */}
      <div className="flex items-center gap-2 px-4 py-2 border-b border-[#1a1a1a] shrink-0 flex-wrap">
      ...
      {/* ─── Filter Drawer ──────────────────────────────────────────────── */}
      {showFilters && (
        <div className="flex flex-wrap items-center gap-2 px-4 py-2 border-b border-[#1a1a1a] bg-[#0a0a0a] shrink-0">
          <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)}
            className="flex-1 sm:flex-none bg-[#111] border border-[#2d2d2d] rounded-lg px-2.5 py-1.5 text-xs text-[#aaa] min-w-0">
      ...
      <div className="flex flex-col md:flex-row md:flex-1 md:overflow-hidden">
        ...
        {showLeft && (
          <div className={`bg-[#0a0a0a] overflow-y-auto md:w-80 md:shrink-0 md:border-r md:border-b-0 border-b border-[#1a1a1a] ${selectedItem ? "hidden md:block" : "block"}`}>
            {loading ? (
        ...
        <div className={`flex flex-col md:flex-1 md:overflow-hidden ${!selectedItem ? "hidden md:flex" : "flex"}`}>
          {/* Mobile back button */}
          {selectedItem && (
            <button
              onClick={() => setSelectedId(null)}
              className="md:hidden flex items-center gap-1.5 px-4 py-2 text-xs text-[#888] border-b border-[#1a1a1a] hover:text-white transition-colors shrink-0"
            >
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {workspaceTab === "summary" && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    {selectedPath && (
                      <div className="col-span-full bg-[#111] border border-[#2d2d2d] rounded-xl p-4">
        ...
        {showRight && selectedItem && (
          <div className="bg-[#0a0a0a] border-t border-[#1a1a1a] md:border-t-0 md:border-l md:w-72 md:shrink-0 md:overflow-y-auto">
