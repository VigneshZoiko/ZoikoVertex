...
export default function BrandStandardsPage() {
  ...
  if (!hasRole(["WORKSPACE_OWNER", "GOVERNANCE_ADMIN", "ADMIN", "BRAND_REVIEWER"])) {
    return <div className="p-8 text-red-400">Unauthorized. You need Brand Reviewer privileges to access Brand Standards.</div>;
  }
      ...
      <div className="flex gap-6 border-b border-[#222]">
        {[
          ...
          <button
            ...
            className={`pb-3 text-sm font-semibold transition-colors border-b-2 ${activeTab === tab.id ? 'border-indigo-500 text-foreground' : 'border-transparent text-[#666] hover:text-[#999]'}`}
          >
            {tab.label}
          </button>
        ))}
            ...
            <div className="space-y-6">
              <div className="bg-[#111] border border-[#222] rounded-xl p-6">
                ...
                <div className="space-y-5">
                  {[
                    { label: "Warmth Index", value: linguistic.warmth_index, color: "bg-orange-500" },
                    { label: "Authority Index", value: linguistic.authority_index, color: "bg-blue-500" },
                    { label: "Restraint Index", value: linguistic.restraint_index, color: "bg-slate-400" },
                    { label: "Cultural Sensitivity", value: linguistic.cultural_sensitivity, color: "bg-emerald-500" },
                    { label: "Evidence Dependency", value: linguistic.evidence_dependency, color: "bg-indigo-500" },
                  ].map(stat => (
                    <div key={stat.label}>
              ...
              <div className="bg-[#111] border border-[#222] rounded-xl p-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {linguistic.prohibited_terms?.map((term: string) => (
                    <span key={term} className="px-3 py-1 bg-rose-500/10 text-rose-400 border border-rose-500/20 rounded-md text-sm font-medium">
                      {term}
                    </span>
                  ))}
                </div>
              ...
              <div className="bg-[#111] border border-[#222] rounded-xl p-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {linguistic.allowed_lexicon?.map((term: string) => (
                    <span key={term} className="px-3 py-1 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 rounded-md text-sm font-medium">
                      {term}
                    </span>
                  ))}
                </div>
        ...
        {activeTab === "claims" && (
          <div className="bg-[#111] border border-[#222] rounded-xl overflow-hidden">
            <div className="p-5 border-b border-[#222] flex items-center justify-between">
              ...
              <button className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-foreground rounded-md text-sm font-medium transition-colors">
                + Register Claim
              </button>
            </div>
                ...
                <tbody className="divide-y divide-[#222]">
                  {claims.map((claim) => (
                    <tr key={claim.id} className="hover:bg-[#161616] transition-colors">
                      ...
                      <td className="p-4 text-sm text-indigo-400 hover:underline cursor-pointer">{claim.source_anchor}</td>
                      <td className="p-4">
                        <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase ${claim.risk_class === 'HIGH' ? 'bg-amber-500/20 text-amber-400' : claim.risk_class === 'REGULATED' ? 'bg-rose-500/20 text-rose-400' : 'bg-[#222] text-[#888]'}`}>
                          {claim.risk_class}
                        </span>
                      </td>
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          <span className={`w-2 h-2 rounded-full ${claim.status === 'ACTIVE' ? 'bg-emerald-500' : 'bg-rose-500'}`} />
