    ...
    <div className="p-8 max-w-7xl mx-auto space-y-12 pb-32 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-6 border-b border-white/5">
        ...
        <button 
          ...
          className="flex items-center justify-center gap-2 px-5 py-3 bg-zinc-900 border border-zinc-800 hover:border-zinc-700 text-white rounded-2xl text-xs font-black uppercase tracking-widest transition-all hover:bg-zinc-800/50"
        >
          ...
          <div className="bg-zinc-950 border border-zinc-900 rounded-[2.5rem] p-8 space-y-6">
            ...
            <div className="grid grid-cols-1 gap-4">
              {profiles.map(prof => (
                <div 
                  key={prof.id} 
                  className="p-5 bg-zinc-900/40 border border-zinc-900 hover:border-pink-500/20 rounded-2xl transition-all group relative overflow-hidden"
                >
                  <div className="flex justify-between items-start">
                    <div>
                      <h4 className="text-base font-black text-white group-hover:text-pink-400 transition-colors flex items-center gap-2">
                        {prof.name}
                        {prof.status === "ACTIVE" && (
                  ...
                  <div className="mt-4 pt-4 border-t border-zinc-900 flex justify-between items-center text-[10px] text-[#555] font-bold uppercase tracking-wider">
                    <span>Target Audience: <strong className="text-zinc-400">{prof.audience}</strong></span>
                    <button className="text-zinc-600 group-hover:text-pink-500 transition-colors flex items-center gap-1">
          ...
          <div className="bg-zinc-950 border border-zinc-900 rounded-[2.5rem] p-8 space-y-6">
            <div className="flex items-center justify-between">
              ...
              <button 
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 bg-zinc-900 border border-zinc-800 hover:border-zinc-700 text-white rounded-xl text-[10px] font-black uppercase tracking-wider transition-all"
              >
            ...
            {showClaimForm && (
              <div className="p-5 bg-zinc-900/60 border border-zinc-900 rounded-2xl space-y-3 animate-in slide-in-from-top duration-300">
                <p className="text-xs font-black text-white uppercase tracking-wider">Configure Substantive Claim</p>
                <textarea 
                  rows={2} 
                  ...
                  className="w-full bg-black border border-zinc-800 rounded-xl px-3 py-2 text-xs text-white placeholder-zinc-600 outline-none focus:border-pink-500/50 resize-none font-medium"
                />
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <input 
                    ...
                    className="bg-black border border-zinc-800 rounded-xl px-3 py-2 text-xs text-white placeholder-zinc-600 outline-none focus:border-pink-500/50"
                  />
                  <select 
                    value={newClaim.category}
                    onChange={e => setNewClaim(f => ({ ...f, category: e.target.value }))}
                    className="bg-black border border-zinc-800 rounded-xl px-3 py-2 text-xs text-white outline-none focus:border-pink-500/50 uppercase tracking-widest font-black"
                  >
                  ...
                  <input 
                    ...
                    className="bg-black border border-zinc-800 rounded-xl px-3 py-2 text-xs text-white outline-none focus:border-pink-500/50"
                  />
                </div>
                <div className="flex justify-end gap-2 pt-2">
                  <button 
                    onClick={() => setShowClaimForm(false)}
                    className="px-3 py-1.5 text-zinc-500 hover:text-white text-xs font-bold transition-all"
                  >
                    Cancel
                  </button>
                  <button 
                    onClick={handleAddClaim}
                    className="px-4 py-1.5 bg-pink-600 hover:bg-pink-500 text-white rounded-xl text-xs font-black uppercase tracking-wider transition-all"
                  >
                    Register Anchor
                  </button>
                </div>
            ...
            <div className="space-y-4">
              {claims.map(claim => (
                <div 
                  key={claim.id} 
                  className="p-5 bg-zinc-900/20 border border-zinc-900 rounded-2xl flex items-start gap-4 hover:border-zinc-800/80 transition-all"
                >
                  <div className="flex-1 space-y-2 min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="text-[9px] font-black bg-zinc-900 border border-zinc-800 text-zinc-400 px-2 py-0.5 rounded uppercase tracking-widest">
                        {claim.category}
                      </span>
                    ...
                    <div className="flex items-center gap-4 text-[10px] text-[#555] font-bold uppercase tracking-wider">
                      <span>Anchor Source: <strong className="text-zinc-400">{claim.anchor}</strong></span>
                      <span>Expires: <strong className="text-zinc-400">{claim.expires}</strong></span>
                    </div>
                  </div>
                  <button 
                    onClick={() => handleDeleteClaim(claim.id, claim.claim)}
                    className="p-1.5 text-zinc-700 hover:text-rose-400 transition-colors shrink-0"
                  >
        ...
        <div className="lg:col-span-5 space-y-8">
          ...
          <div className="bg-zinc-950 border border-zinc-900 rounded-[2.5rem] p-8 space-y-8">
            <div>
              ...
              <p className="text-[10px] text-zinc-500 uppercase font-bold tracking-wider mt-2">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs font-bold uppercase tracking-widest">
                  <span className="text-zinc-300 flex items-center gap-1.5 cursor-help" title="Measures empathy, supportiveness, and informal brand tone.">
                    Warmth Index
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs font-bold uppercase tracking-widest">
                  <span className="text-zinc-300 flex items-center gap-1.5 cursor-help" title="Decisiveness, expert lexicon density, and technical posture.">
                    Authority Index
                    <HelpCircle className="w-3.5 h-3.5 text-zinc-600" />
                  </span>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs font-bold uppercase tracking-widest">
                  <span className="text-zinc-300 flex items-center gap-1.5 cursor-help" title="Caution level, competitive non-disclosure, and compliance posture.">
                    Restraint Index
                    <HelpCircle className="w-3.5 h-3.5 text-zinc-600" />
                  </span>
          ...
          <div className="bg-zinc-950 border border-zinc-900 rounded-[2.5rem] p-8 space-y-6">
            ...
            <div className="flex gap-2">
              <input 
                ...
                className="flex-1 bg-black border border-zinc-800 rounded-xl px-3 py-2 text-xs text-white placeholder-zinc-600 outline-none focus:border-pink-500/50"
              />
              <select 
                value={termType}
                onChange={e => setTermType(e.target.value as any)}
                className="bg-zinc-900 border border-zinc-800 rounded-xl px-2 text-xs text-white outline-none font-bold uppercase"
              >
              ...
              <button 
                ...
                className="p-2.5 bg-pink-600 hover:bg-pink-500 text-white rounded-xl transition-all"
              >
