    ...
    return (
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-16 text-center">
          ...
          <p className="text-gray-900 dark:text-white font-semibold mb-2">Failed to Load Quality Audit</p>
          <p className="text-[#666] text-sm mb-6">{error}</p>
          <button onClick={fetchAll} className="px-5 py-2.5 bg-indigo-500 hover:bg-indigo-400 text-gray-900 dark:text-white rounded-xl text-xs font-bold transition-all">
            Retry
          </button>
        </div>
      </div>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6 pb-16">
      ...
      <div className="mb-5 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-1">Quality Audit</h1>
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          <TooltipBtn disabled={!selectedItem || actionLoading !== null} tooltip="Select an audit item first">
            <button disabled={!selectedItem || actionLoading !== null}
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-500 hover:bg-indigo-400 text-gray-900 dark:text-white rounded-lg text-[10px] font-bold transition-all disabled:opacity-40 shadow-lg shadow-indigo-500/15">
        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            <input type="text" placeholder="Search audit items…" value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
              className="w-48 pl-8 pr-3 py-1.5 bg-[var(--card)] border border-[var(--border)] rounded-lg text-xs text-gray-900 dark:text-white placeholder-[#555] outline-none focus:border-indigo-500/40" />
          </div>
        ...
        <div className="xl:col-span-3 space-y-2">
          ...
          {filteredItems.length === 0 ? (
            <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl p-8 text-center">
              {activeTab === "assigned_to_me" ? <><UserCheck className="w-6 h-6 text-indigo-400 mx-auto mb-3" /><p className="text-xs text-gray-900 dark:text-white font-semibold mb-1">No Assigned Items</p><p className="text-[10px] text-[#666]">Audit items assigned to you will appear here.</p></> :
               activeTab === "passed" ? <><CheckCircle2 className="w-6 h-6 text-emerald-400 mx-auto mb-3" /><p className="text-xs text-gray-900 dark:text-white font-semibold mb-1">No Passed Audits</p><p className="text-[10px] text-[#666]">Items that passed quality audit will appear here.</p></> :
               activeTab === "failed" ? <><XCircle className="w-6 h-6 text-rose-400 mx-auto mb-3" /><p className="text-xs text-gray-900 dark:text-white font-semibold mb-1">No Failed Audits</p><p className="text-[10px] text-[#666]">Items that failed quality audit will appear here with defect details.</p></> :
               activeTab === "needs_correction" ? <><AlertTriangle className="w-6 h-6 text-orange-400 mx-auto mb-3" /><p className="text-xs text-gray-900 dark:text-white font-semibold mb-1">No Corrections Needed</p><p className="text-[10px] text-[#666]">Items requiring corrective action will appear here.</p></> :
               activeTab === "high_severity" ? <><AlertCircle className="w-6 h-6 text-red-400 mx-auto mb-3" /><p className="text-xs text-gray-900 dark:text-white font-semibold mb-1">No High-Severity Defects</p><p className="text-[10px] text-[#666]">Items with Major or Critical defects will appear here.</p></> :
               activeTab === "published_check" ? <><Send className="w-6 h-6 text-blue-400 mx-auto mb-3" /><p className="text-xs text-gray-900 dark:text-white font-semibold mb-1">No Published Checks</p><p className="text-[10px] text-[#666]">Published and sent items selected for version-matching review will appear here.</p></> :
               activeTab === "completed" ? <><CheckCircle2 className="w-6 h-6 text-gray-400 mx-auto mb-3" /><p className="text-xs text-gray-900 dark:text-white font-semibold mb-1">No Completed Audits</p><p className="text-[10px] text-[#666]">Closed and archived audits will appear here.</p></> :
               <><ShieldCheck className="w-6 h-6 text-emerald-400 mx-auto mb-3" /><p className="text-xs text-gray-900 dark:text-white font-semibold mb-1">No Items Selected for Audit</p><p className="text-[10px] text-[#666]">Generate a sample or select items from completed workflows to begin quality auditing.</p></>}
            </div>
          ) : (
                      ...
                      <div className="flex items-center gap-1.5">
                        {bulkMode && (
                          <div className={`w-4 h-4 rounded border-2 flex items-center justify-center shrink-0 transition-all ${isBulk ? "bg-indigo-500 border-indigo-500" : "border-[#555]"}`}>
                            {isBulk && <CheckCircle2 className="w-3 h-3 text-gray-900 dark:text-white" />}
                          </div>
                        )}
        ...
        <div className="xl:col-span-5">
          {!selectedItem ? (
            <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-16 text-center">
              ...
              <p className="text-gray-900 dark:text-white font-semibold mb-1">Select an Audit Item</p>
            ...
            <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-4 space-y-4">
              ...
              <div className="flex items-center justify-between mb-2">
                <div>
                  <h3 className="text-sm font-bold text-gray-900 dark:text-white">{selectedItem.title}</h3>
                      ...
                      defects.map(d => (
                        <div key={d.id} className="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-3">
                          <div className="flex items-center justify-between mb-1">
                            <span className="text-[11px] text-gray-900 dark:text-white font-medium">{d.category}</span>
                            <div className="flex items-center gap-1.5">
                    ...
                    {evidence.length === 0 ? (
                      ...
                      evidence.map(e => (
                        <div key={e.id} className="flex items-center justify-between bg-[var(--surface)] border border-[var(--border)] rounded-xl p-3">
                          <div>
                            <span className="text-[11px] text-gray-900 dark:text-white font-medium">{e.evidence_type}</span>
                    ...
                    {correctiveActions.length === 0 ? (
                      ...
                      correctiveActions.map(ca => (
                        <div key={ca.id} className="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-3">
                          <div className="flex items-center justify-between mb-1">
                            <span className="text-[11px] text-gray-900 dark:text-white font-medium">{ca.title}</span>
              ...
              <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl p-4">
                ...
                {actionLoading && (
                  <textarea placeholder="Reason (required for fail, correction, escalation)…" value={feedbackText} onChange={e => setFeedbackText(e.target.value)}
                    className="w-full mt-2 bg-[var(--surface)] border border-[var(--border)] rounded-lg p-2 text-[11px] text-gray-900 dark:text-white placeholder-[#555] outline-none focus:border-indigo-500/30 resize-none" rows={2} />
                )}
              </div>
                      ...
                      <div key={d.id} className="flex items-start gap-2 p-2 bg-[var(--surface)] border border-[var(--border)] rounded-lg">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between">
                            <span className="text-[10px] text-gray-900 dark:text-white font-medium truncate">{d.category}</span>
                  ...
                  <div className="space-y-1.5 max-h-[120px] overflow-y-auto">
                    {correctiveActions.map(ca => (
                      <div key={ca.id} className="p-2 bg-[var(--surface)] border border-[var(--border)] rounded-lg">
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] text-gray-900 dark:text-white font-medium truncate">{ca.title}</span>
              ...
              <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl p-4">
                    ...
                    {notes.map(n => (
                      <div key={n.id} className="p-2 bg-[var(--surface)] border border-[var(--border)] rounded-lg">
                        <div className="flex items-center justify-between mb-0.5">
                          <span className="text-[10px] font-medium text-gray-900 dark:text-white">{n.created_by}</span>
                ...
                <div className="flex gap-2">
                  <input type="text" placeholder="Add a note…" value={feedbackText} onChange={e => setFeedbackText(e.target.value)}
                    className="flex-1 bg-[var(--surface)] border border-[var(--border)] rounded-lg px-2.5 py-1.5 text-[11px] text-gray-900 dark:text-white placeholder-[#555] outline-none focus:border-indigo-500/30" />
                  <button onClick={handleAddNote} disabled={actionLoading === "add_note" || !feedbackText.trim()}
                    className="px-2.5 py-1.5 bg-indigo-500 hover:bg-indigo-400 text-gray-900 dark:text-white rounded-lg text-[10px] font-bold transition-all disabled:opacity-40">
                  ...
                  <div className="space-y-1.5 max-h-[120px] overflow-y-auto">
                    {evidence.map(e => (
                      <div key={e.id} className="flex items-center justify-between p-2 bg-[var(--surface)] border border-[var(--border)] rounded-lg">
                        <div className="flex items-center gap-2">
                          <span className="text-[10px] text-gray-900 dark:text-white">{e.evidence_type}</span>
