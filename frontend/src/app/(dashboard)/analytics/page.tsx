...
const PLATFORM_META: Record<string, { label: string; color: string; dot: string; iconBg: string }> = {
  ...
  threads:   { label: "Threads",   color: "text-gray-700 dark:text-zinc-300",   dot: "bg-zinc-400",   iconBg: "bg-zinc-500/15" },
  twitter:   { label: "Twitter/X", color: "text-gray-500 dark:text-zinc-400",   dot: "bg-zinc-500",   iconBg: "bg-zinc-500/15" },
};
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {Object.entries(grouped).map(([platform, accounts]) => {
                  const meta = PLATFORM_META[platform] ?? { label: platform, color: "text-gray-500 dark:text-zinc-400", dot: "bg-zinc-500" };
