  ...
  const btn = (key: string, label: string, icon: React.ReactNode, style: string, disabled?: boolean) => (
    <button
      ...
      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all disabled:opacity-40 disabled:cursor-not-allowed ${
        style === "primary"
          ? "bg-indigo-500 hover:bg-indigo-400 text-foreground shadow-lg shadow-indigo-500/20"
          : style === "danger"
    ...
    <div className="space-y-3">
      ...
      {["reject", "request_revision", "escalate", "override"].includes(actionLoading?.replace(item.id, "") || "") && (
        <textarea
          ...
          rows={2}
          className="w-full bg-[var(--surface)] border border-[var(--border)] rounded-lg p-2.5 text-xs text-foreground placeholder-[#555] outline-none focus:border-indigo-500/50 resize-none"
        />
      )}
    </div>
  );
      ...
      case "Agent Action":
        return (
          <div className="space-y-2 text-sm">
            <div className="flex justify-between text-xs"><span className="text-[#888]">Agent:</span><span className="text-foreground">{(item.content_snapshot?.agent_name as string) || "—"}</span></div>
            <div className="flex justify-between text-xs"><span className="text-[#888]">Task:</span><span className="text-foreground">{(item.content_snapshot?.task as string) || "—"}</span></div>
            <div className="flex justify-between text-xs"><span className="text-[#888]">Proposed Action:</span><span className="text-foreground">{(item.content_snapshot?.proposed_action as string) || "—"}</span></div>
            <div className="flex justify-between text-xs"><span className="text-[#888]">Affected Module:</span><span className="text-foreground">{item.source_module}</span></div>
          </div>
        );
      case "Campaign Asset":
        return (
          <div className="space-y-2 text-sm">
            <div className="flex justify-between text-xs"><span className="text-[#888]">Campaign:</span><span className="text-foreground">{item.campaign || "—"}</span></div>
            <div className="flex justify-between text-xs"><span className="text-[#888]">Objective:</span><span className="text-foreground">{(item.content_snapshot?.objective as string) || "—"}</span></div>
            <div className="flex justify-between text-xs"><span className="text-[#888]">Audience:</span><span className="text-foreground">{(item.content_snapshot?.audience as string) || "—"}</span></div>
            <div className="flex justify-between text-xs"><span className="text-[#888]">CTA:</span><span className="text-foreground">{(item.content_snapshot?.cta as string) || "—"}</span></div>
          </div>
        );
      case "Workflow Output":
        return (
          <div className="space-y-2 text-sm">
            <div className="flex justify-between text-xs"><span className="text-[#888]">Workflow:</span><span className="text-foreground">{item.source_module}</span></div>
            <div className="flex justify-between text-xs"><span className="text-[#888]">Trigger:</span><span className="text-foreground">{(item.content_snapshot?.triggering_event as string) || "—"}</span></div>
            <div className="flex justify-between text-xs"><span className="text-[#888]">Output:</span><span className="text-foreground">{(item.content_snapshot?.output as string) || item.title}</span></div>
          </div>
        );
      default:
    ...
    return (
      <div className="max-w-5xl mx-auto pb-16 px-4">
        <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-16 text-center">
          ...
          <p className="text-foreground font-semibold mb-1">Review Queue could not be loaded</p>
          ...
          <button
            onClick={fetchAll}
            className="px-4 py-2 bg-indigo-500 hover:bg-indigo-400 text-foreground rounded-lg text-sm font-semibold transition-all"
          >
            Try Again
          </button>
        </div>
  ...
  return (
    <div className="pb-16 px-4">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground mb-1">Review Queue</h1>
        ...
        <div className="w-[340px] shrink-0 flex flex-col gap-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2.5 text-xs rounded-xl bg-[var(--card)] border border-[var(--border)] text-foreground placeholder-[#555] outline-none focus:border-indigo-500/40"
            />
          </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-2 pr-1 scrollbar-thin">
            {loading ? (
              ...
              <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-8 text-center">
                ...
                <p className="text-foreground font-semibold text-sm mb-1">Failed to load queue</p>
        ...
        <div className="flex-1 min-w-0">
          {!selectedItem ? (
            <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl h-full min-h-[400px] flex flex-col items-center justify-center p-12">
              ...
              <p className="text-foreground font-semibold mb-1">Select an item to review</p>
            ...
            <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl overflow-hidden h-full">
              ...
              <div className="p-4 border-b border-[var(--border)]/50">
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <h3 className="text-sm font-bold text-foreground mb-0.5">{detailItem?.title}</h3>
        ...
        <div className="w-[320px] shrink-0 flex flex-col gap-3 overflow-y-auto">
          ...
          {detailItem && (
            <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h4 className="text-[11px] font-bold text-foreground uppercase tracking-wider">Risk Summary</h4>
              </div>
              <div className="space-y-2 text-[11px]">
                <div className="flex justify-between items-center">
                  <span className="text-[#888]">Overall Risk</span>
                  <span className={`font-bold ${RISK_CONFIG[(detailItem as ReviewItem).risk_level as keyof typeof RISK_CONFIG]?.color || "text-foreground"}`}>
                ...
                {detailItem.risk_category && (
                  <div className="flex justify-between">
                    ...
                    <span className="text-foreground text-right max-w-[180px]">{detailItem.risk_category}</span>
                  </div>
                )}
                {detailItem.confidence_score !== undefined && (
                  <div className="flex justify-between">
                    <span className="text-[#888]">Confidence</span>
                    <span className="text-foreground">{(detailItem.confidence_score * 100).toFixed(0)}%</span>
                  </div>
                )}
                {eligibility?.state && (
                  <div className="flex justify-between">
                    <span className="text-[#888]">Eligibility</span>
                    <span className="text-foreground text-right max-w-[180px]">{eligibility.state}</span>
                  </div>
                )}
              </div>
          ...
          {detailItem && (
            <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h4 className="text-[11px] font-bold text-foreground uppercase tracking-wider">Validation Summary</h4>
              </div>
          ...
          {detailItem && (
            <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h4 className="text-[11px] font-bold text-foreground uppercase tracking-wider">Assignment</h4>
              </div>
              <div className="space-y-2 text-[11px]">
                <div className="flex justify-between">
                  <span className="text-[#888]">Assigned To</span>
                  <span className="text-foreground">{detailItem.assigned_to?.slice(0, 12) || "Unassigned"}</span>
                </div>
                {detailItem.assigned_team && (
                  <div className="flex justify-between">
                    <span className="text-[#888]">Team</span>
                    <span className="text-foreground">{detailItem.assigned_team}</span>
                  </div>
                )}
                <div className="flex justify-between">
                  <span className="text-[#888]">Submitted By</span>
                  <span className="text-foreground">{detailItem.submitted_by?.slice(0, 12)}</span>
                </div>
                {detailItem.due_at && (
                  <div className="flex justify-between">
          ...
          {detailItem && (
            <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h4 className="text-[11px] font-bold text-foreground uppercase tracking-wider">Reviewer Notes</h4>
              </div>
              ...
              <div className="flex gap-2">
                <textarea
                  ...
                  rows={2}
                  className="flex-1 bg-[var(--surface)] border border-[var(--border)] rounded-lg p-2 text-[11px] text-foreground placeholder-[#555] outline-none focus:border-indigo-500/50 resize-none"
                />
                <button
                  onClick={handleAddNote}
                  disabled={actionLoading === "note" || !feedback.trim()}
                  className="px-2.5 py-1.5 bg-indigo-500 hover:bg-indigo-400 disabled:opacity-50 text-foreground rounded-lg text-[10px] font-bold transition-all shrink-0 self-end"
                >
          ...
          {detailItem && (
            <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h4 className="text-[11px] font-bold text-foreground uppercase tracking-wider">Audit Timeline</h4>
              </div>
          ...
          {!detailItem && (
            <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-8 text-center">
              ...
              <p className="text-foreground text-xs font-semibold mb-1">No Item Selected</p>
      ...
      {noteDialogItemId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => { setNoteDialogItemId(null); setNoteDialogText(""); }}>
          <div className="bg-[#0a0a0a] border border-[#2d2d2d] rounded-2xl p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
            <h3 className="text-sm font-bold text-foreground mb-4">Add Note</h3>
            <textarea value={noteDialogText} onChange={e => setNoteDialogText(e.target.value)}
              rows={4} className="w-full p-3 bg-[#111] border border-[#2d2d2d] rounded-lg text-xs text-foreground placeholder:text-[#555] focus:outline-none focus:border-indigo-500/40 resize-none"
            ...
            <div className="flex justify-end gap-2 mt-5 pt-4 border-t border-[#2d2d2d]">
              ...
              <button onClick={() => { if (noteDialogText.trim() && noteDialogItemId) { handleAction("add-note", noteDialogItemId); setNoteDialogItemId(null); setNoteDialogText(""); } }} disabled={!noteDialogText.trim() || actionLoading === "add-note"} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 disabled:bg-indigo-800 text-foreground rounded-lg text-xs font-medium">Add Note</button>
            </div>
          </div>
        </div>
      )}
