  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-bold text-white mb-2">Common Media Library</h1>
          <p className="text-slate-400">Browse and pick assets uploaded by Creators to start publishing.</p>
        </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-8">
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full bg-slate-900 border border-slate-800 rounded-2xl py-4 pl-12 pr-4 text-white focus:ring-2 focus:ring-indigo-500 transition-all outline-none"
          />
        </div>
          ...
          {['all', 'image', 'video'].map((t) => (
            <button
              ...
              className={`px-6 py-3 rounded-xl text-sm font-medium transition-all ${
                filter === t ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20' : 'text-slate-400 hover:text-white'
              }`}
            >
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {assets.map((asset) => (
            <div key={asset.id} className="group relative bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden hover:border-indigo-500/50 transition-all hover:shadow-2xl hover:shadow-indigo-500/10">
              ...
              {/* Asset Info */}
              <div className="p-4">
                <h3 className="text-white font-semibold truncate mb-1">{asset.title}</h3>
        ...
        <div className="text-center py-24 bg-slate-900/30 rounded-3xl border border-dashed border-slate-800">
          <div className="w-20 h-20 bg-slate-900 rounded-3xl flex items-center justify-center mx-auto mb-6">
            <Filter className="text-slate-700 w-10 h-10" />
          </div>
          <h3 className="text-xl font-bold text-white mb-2">No assets found</h3>
          <p className="text-slate-500 max-w-sm mx-auto">Try adjusting your search or ask Creators to upload new content.</p>
        </div>
      )}
    </div>
  );
