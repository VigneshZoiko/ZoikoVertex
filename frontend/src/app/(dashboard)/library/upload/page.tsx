                    ...
                    {entries.map((entry, idx) => (
                      <div key={idx} className="relative aspect-video rounded-xl overflow-hidden bg-black/60 border border-[var(--border)] group/thumb">
                          ...
                          <>
                            ...
                            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                              <div className="w-10 h-10 bg-black/60 rounded-full flex items-center justify-center backdrop-blur-sm">
                                <Play className="w-4 h-4 text-gray-900 dark:text-white fill-white ml-0.5" />
                              </div>
                            </div>
                          </>
                        ) : (
                        ...
                        <div className={`absolute top-2 left-2 flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold backdrop-blur-sm ${
                          isVideo(entry.file) ? 'bg-violet-600/80 text-gray-900 dark:text-white' : 'bg-sky-600/80 text-gray-900 dark:text-white'
                        }`}>
                        ...
                        {!isUploading && (
                          <button
                            ...
                            className="absolute top-2 right-2 w-6 h-6 rounded-full bg-rose-600 text-gray-900 dark:text-white flex items-center justify-center opacity-0 group-hover/thumb:opacity-100 transition-all hover:bg-rose-500 hover:scale-110 shadow-lg z-10"
                          >
                            <X className="w-3 h-3" />
                          </button>
                        )}
                        ...
                        {entry.status === 'uploading' && (
                          <div className="absolute inset-x-0 bottom-0">
                            ...
                            <div className="bg-black/60 backdrop-blur text-center text-[10px] text-gray-900 dark:text-white py-1">
                              {entry.progress}%
                            </div>
                          </div>
                        )}
                        ...
                        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent p-2 opacity-0 group-hover/thumb:opacity-100 transition-opacity pointer-events-none">
                          <p className="text-[10px] text-gray-900 dark:text-white truncate font-medium">{entry.file.name}</p>
          ...
          <button
            ...
            className={`w-full py-4 rounded-2xl font-bold flex items-center justify-center gap-2 transition-all ${
              isUploading || entries.length === 0 || !title.trim()
                ...
                : 'bg-gradient-to-r from-indigo-600 to-violet-600 text-gray-900 dark:text-white hover:shadow-lg hover:shadow-indigo-500/20 active:scale-[0.98]'
            }`}
          >
            {isUploading ? (
              <>
