...
const STATUS_CONFIG: Record<string, { label: string; color: string }> = {
  ...
  RELEASED: {
    label: "Released",
    color: "bg-emerald-600/10 text-emerald-300 border-emerald-600/20",
  },
};
...
export default function ReviewQueuePage() {
  ...
  const [showActionDrawer, setShowActionDrawer] = useState<
    "reject" | "return" | null
  >(null);
  ...
  const handleAction = async (
    ...
    try {
      ...
      if (r.success) {
        ...
        setMessage({
          type: r.blocked ? "error" : "success",
          text: labels[action] || "Done.",
        });
        setShowActionDrawer(null);
        setDrawerText("");
    ...
    .filter((item) => {
      ...
      if (activeTab === "resolve")
        return (
          ...
          (item.status === "IN_REVIEW" || item.status === "ASSIGNED")
        );
  ...
  const sorted = [...filtered].sort((a, b) => {
    ...
    return (
      (p[a.priority as keyof typeof p] ?? 9) -
      (p[b.priority as keyof typeof p] ?? 9)
    );
  });
  ...
  // Media
  const mediaUrls = selectedItem?.content_snapshot?.urls as
    | string[]
    | undefined;
  const fileType = selectedItem?.content_snapshot?.file_type as
    | string
    | undefined;
  ...
  const isPoolItem =
    selectedItem?.status === "PENDING_REVIEW" && !selectedItem?.assigned_to;
  const isResolved =
    selectedItem &&
    ["APPROVED", "REJECTED", "AWAITING_REVISION", "RELEASED"].includes(
      selectedItem.status,
      ...
      {stats && (
        <div className="grid grid-cols-4 gap-4 mb-8">
          {[
            ...
            {
              label: "Returned",
              value: stats.rejected + stats.awaiting_revision,
              color: "text-rose-400",
            },
          ].map((s) => (
            <div
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1.5 mb-1.5 flex-wrap">
                      <span
                        className={`px-1.5 py-[1px] rounded border text-[8px] font-bold ${status.color}`}
                      >
                        {status.label}
                      </span>
                      <span
                        className={`px-1.5 py-[1px] rounded border text-[8px] font-bold ${RISK_CONFIG[item.risk_level] || ""}`}
                      >
                        {item.risk_level}
                      </span>
                    </div>
                  ...
                  >
                    ...
                    <div className="pb-3 border-b border-border">
                      ...
                      <div className="flex items-center gap-1.5 flex-wrap mb-2">
                        ...
                        <span
                          className={`px-2 py-0.5 rounded border text-[8px] font-bold ${RISK_CONFIG[selectedItem.risk_level] || ""}`}
                        >
                    ...
                    {violation && (
                      <div className="flex items-start gap-2.5 p-2.5 bg-red-500/[0.05] border border-red-500/20 rounded-lg">
                        ...
                        <div>
                          <p className="text-[9px] font-bold text-red-300 mb-0.5 uppercase tracking-wide">
                            Block Reason
                          </p>
              ...
              {showActionDrawer && (
                ...
                >
                  <div className="flex items-center justify-between">
                    <p
                      className={`text-xs font-bold ${showActionDrawer === "reject" ? "text-red-300" : "text-orange-300"}`}
                    >
                      {showActionDrawer === "reject"
                        ? "Rejection Reason"
                        : "Return Instructions"}
