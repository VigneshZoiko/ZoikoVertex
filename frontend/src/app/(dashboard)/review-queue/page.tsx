...
export default function ReviewQueuePage() {
  ...
  const handleAction = async (action: string, extra?: Record<string, string>) => {
    ...
    setMessage(null);
    try {
      const r = await api.post(`/api/v1/review-queue/items/${selectedItem.id}/action`, { action, ...(extra || {}) });
      if (r.success) {
        const labels: Record<string, string> = {
          claim: "Item claimed.", unclaim: "Released to pool.",
          approve: "Approved. Creator notified.", reject: "Rejected. Creator notified.",
