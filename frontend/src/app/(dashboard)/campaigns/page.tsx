...
export default function CampaignsPage() {
  ...
  const [bulkPixelOpen,   setBulkPixelOpen]   = useState(false);
  const [bulkPixels,      setBulkPixels]      = useState<{ id: string; name: string }[]>([]);
  const [bulkPxLoading,   setBulkPxLoading]   = useState(false);
  const [bulkPixelId,     setBulkPixelId]     = useState("");
  const [bulkPixelSearch, setBulkPixelSearch]  = useState("");
  ...
  return (
      ...
      <div className="flex-1 min-w-0 flex flex-col overflow-hidden">
        ...
        <div className="flex flex-wrap items-center justify-between gap-y-2 px-4 sm:px-6 py-3.5 border-b border-border/60 bg-card">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {/* Bulk Assign Pixel */}
            {tab === "ad-campaigns" && compareMode && compareIds.size > 0 && (
              <button
                onClick={() => {
                  setBulkPixelId("");
                  setBulkPixelSearch("");
                  setBulkResult(null);
                  setBulkPxLoading(true);
                  api.get("/api/v1/campaigns/meta/pixels").then(r => {
                    setBulkPixels(r.data?.pixels || []);
                    setBulkPxLoading(false);
                  }).catch(() => setBulkPxLoading(false));
                  setBulkPixelOpen(true);
                }}
                className="flex items-center gap-1.5 px-3 py-2 border text-xs font-semibold rounded-lg transition-colors bg-blue-500/10 border-blue-500/30 text-blue-400 hover:bg-blue-500/20"
              >
                <Zap className="w-3.5 h-3.5" />
                Assign Pixel ({compareIds.size})
              </button>
            )}
    ...
    />
      ...
      {/* Bulk Assign Pixel Modal */}
      {bulkPixelOpen && (
        <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/70 backdrop-blur-sm px-4" onClick={() => { if (!bulkApplying) setBulkPixelOpen(false); }}>
          <div className="w-full max-w-md bg-card border border-border rounded-2xl shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-4 border-b border-border">
              <div>
                <h3 className="text-base font-bold text-foreground">Assign Pixel to Campaigns</h3>
                <p className="text-xs text-foreground-muted mt-0.5">{compareIds.size} campaign{compareIds.size !== 1 ? "s" : ""} selected</p>
              </div>
              <button onClick={() => setBulkPixelOpen(false)} className="p-1.5 text-foreground-muted hover:text-white hover:bg-surface-hover rounded-lg transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="p-5 space-y-4">
              <div>
                <p className="text-xs font-semibold text-foreground-muted mb-1.5">Select a Meta Pixel</p>
                <div className="relative">
                  <input
                    type="text"
                    value={bulkPixelSearch}
                    onChange={e => setBulkPixelSearch(e.target.value)}
                    placeholder="Search or paste Pixel ID\u2026"
                    className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm text-foreground placeholder:text-foreground-muted focus:outline-none focus:border-white/30"
                    autoFocus
                  />
                </div>
              </div>
              <div className="max-h-56 overflow-y-auto border border-border rounded-xl divide-y divide-border/60">
                {bulkPxLoading ? (
                ...
                ) : bulkPixels.length === 0 && bulkPixelSearch.length === 0 ? (
                  ...
                  (bulkPixelSearch
                    ? bulkPixels.filter(p => p.id.includes(bulkPixelSearch) || p.name.toLowerCase().includes(bulkPixelSearch.toLowerCase()))
                    : bulkPixels
                  ).map(px => (
                    <button
                      key={px.id}
                      type="button"
                      onClick={() => {
                        setBulkPixelId(px.id);
                        setBulkPixelSearch(px.name + " (" + px.id + ")");
                      }}
                      className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-white/5 ${
                        bulkPixelId === px.id ? "bg-blue-500/10" : ""
                      }`}
                    >
                      <div className={`w-4 h-4 rounded-full border-2 shrink-0 flex items-center justify-center ${
                        bulkPixelId === px.id ? "border-blue-400" : "border-border"
                      }`}>
                        {bulkPixelId === px.id && <div className="w-2 h-2 rounded-full bg-blue-400" />}
                      </div>
              ...
              {bulkResult && (
                <div className={`flex items-start gap-2 px-3 py-2.5 rounded-xl text-xs font-medium ${
                  bulkResult.ok ? "bg-success-text/10 text-success-text border border-success-text/20" : "bg-error-text/10 text-error-text border border-error-text/20"
                }`}>
                  {bulkResult.ok ? <CheckCircle2 className="w-3.5 h-3.5 shrink-0 mt-0.5" /> : <AlertCircle className="w-3.5 h-3.5 shrink-0 mt-0.5" />}
                  {bulkResult.text}
                </div>
              )}
              <div className="flex justify-end gap-2 pt-1">
                <button onClick={() => setBulkPixelOpen(false)} disabled={bulkApplying}
                ...
                <button
                  onClick={async () => {
                    if (!bulkPixelId || bulkApplying) return;
                    setBulkApplying(true);
                    setBulkResult(null);
                    try {
                      const r = await api.post("/api/v1/campaigns/bulk/assign-pixel", {
                        campaign_ids: Array.from(compareIds),
                        tracking_pixel_id: bulkPixelId,
                      });
                      if (r.success) {
                        setBulkResult({ ok: true, text: "Pixel assigned to " + (r.data?.updated_count ?? compareIds.size) + " campaign" + ((r.data?.updated_count ?? compareIds.size) !== 1 ? "s." : ".") });
                        setTimeout(() => { load(); }, 500);
                      } else {
                        setBulkResult({ ok: false, text: r.error || "Failed to assign pixel" });
                      }
                    } catch {
                      setBulkResult({ ok: false, text: "Request failed. Check your connection." });
                    } finally {
                      setBulkApplying(false);
                    }
                  }}
                  disabled={!bulkPixelId || bulkApplying}
                  className="flex items-center gap-2 px-5 py-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white text-sm font-semibold rounded-xl transition-colors">
                  {bulkApplying && <Loader2 className="w-4 h-4 animate-spin" />}
                  {bulkApplying ? "Assigning\u2026" : "Assign to " + compareIds.size + " campaign" + (compareIds.size !== 1 ? "s" : "")}
                </button>
              </div>
            </div>
          </div>
