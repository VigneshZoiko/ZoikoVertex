...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      <div className="w-64 bg-zinc-950 border-r border-zinc-800/70 flex flex-col h-screen">
        {/* ── Brand ── */}
        <div className="flex items-center px-5 py-5 border-b border-zinc-800/70">
          ...
          <span className="text-white font-bold text-[17px] tracking-wide">ZoikoVertex</span>
        </div>

        {/* ── Nav ── */}
        <div className="flex-1 overflow-y-auto py-4 space-y-1 scrollbar-hide px-3">
          {!roleLoaded ? (
            <div className="space-y-2 px-2 pt-2">
              {[1, 2, 3, 4, 5].map(i => (
                <div key={i} className="h-9 rounded-lg bg-zinc-800/50 animate-pulse" />
              ))}
            </div>
          ) : (
            NAV_GROUPS.map(group => {
              ...
              const groupActive = visibleItems.some(i => pathname === i.href);
              ...
              return (
                <div key={group.id}>
                  ...
                  >
                    <GroupIcon className={`w-4 h-4 mr-2.5 flex-shrink-0 ${groupActive ? "text-indigo-400" : "text-zinc-500 group-hover:text-zinc-300"}`} />
                  ...
                  {/* Group items */}
                  {isOpen && (
                    <div className="mt-0.5 ml-3 pl-3 border-l border-zinc-800/60 space-y-0.5">
                      {visibleItems.map(item => {
                        const Icon     = item.icon;
                        const isActive = pathname === item.href;
                        const showBadge = item.badge && pendingCount > 0;
                        const showDot   = item.dirty && isDirty;
                          ...
                          <a
                            ...
                            className={`flex items-center px-3 py-2 rounded-lg transition-colors group text-sm
                              ${isActive
                                ...
                                : "text-zinc-400 hover:text-white hover:bg-zinc-800/40"}`}
                          >
                            <Icon className={`w-4 h-4 mr-2.5 flex-shrink-0 ${isActive ? "text-indigo-400" : "text-zinc-500 group-hover:text-zinc-300"}`} />
                            ...
                            {showDot && (
                              <span className="ml-2 w-2 h-2 rounded-full bg-amber-400 animate-pulse shadow-lg shadow-amber-400/20" title="Unsaved draft" />
                            )}
                          </a>
                        );
                      })}
        ...
        {/* ── User / Logout ── */}
        <div className="border-t border-zinc-800/70 px-3 py-4 space-y-2">
          <div className="flex items-center px-3 py-2.5 rounded-xl bg-zinc-900 border border-zinc-800">
            <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-indigo-500 to-purple-500 border-2 border-zinc-950 flex-shrink-0" />
            <div className="ml-3 min-w-0">
              <p className="text-sm font-medium text-white leading-none truncate">Agent Profile</p>
