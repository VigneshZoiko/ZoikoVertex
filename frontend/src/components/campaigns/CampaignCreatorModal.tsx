...
const inp = "w-full bg-gray-50 dark:bg-zinc-900 border border-gray-300 dark:border-zinc-700 rounded-lg px-3 py-2 text-sm text-gray-900 dark:text-white placeholder:text-gray-400 dark:text-zinc-600 focus:outline-none focus:border-gray-400 dark:focus:border-zinc-500 transition-colors";

const Field = React.memo(function Field({ label, hint, children }: { label: string; hint?: string; children: React.ReactNode }) {
  return (
    <div className="space-y-1.5">
      <p className="text-sm font-semibold text-gray-800 dark:text-zinc-200">{label}</p>
      {hint && <p className="text-xs text-gray-500 dark:text-zinc-500">{hint}</p>}
      {children}
    </div>
  );
});
...
const Radio = React.memo(function Radio({ value, checked, onChange, label, desc }: { value: string; checked: boolean; onChange: () => void; label: string; desc?: string }) {
  return (
    <label className="flex items-start gap-3 cursor-pointer group py-1">
      <div className={`mt-0.5 w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${checked ? "border-white" : "border-gray-400 dark:border-zinc-600 group-hover:border-zinc-400"}`}>
        {checked && <div className="w-2 h-2 rounded-full bg-white" />}
      </div>
      <input type="radio" className="hidden" value={value} checked={checked} onChange={onChange} />
      <div>
        <p className={`text-sm font-medium ${checked ? "text-gray-900 dark:text-white" : "text-gray-700 dark:text-zinc-300"}`}>{label}</p>
        {desc && <p className="text-xs text-gray-500 dark:text-zinc-500 mt-0.5">{desc}</p>}
      </div>
    </label>
  );
});
...
const MessageDestPicker = React.memo(function MessageDestPicker({ msgDest, setMsgDest }: { msgDest: string; setMsgDest: (v: string) => void }) {
  return (
    <div className="ml-7 mt-2 space-y-1.5">
      <p className="text-xs font-semibold text-gray-500 dark:text-zinc-400">Select destination</p>
      <div className="border border-gray-300 dark:border-zinc-700 rounded-xl overflow-hidden divide-y divide-gray-200 dark:divide-zinc-800">
        <label className="flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-gray-200 dark:bg-zinc-800/40 transition-colors">
          <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 ${msgDest === "messenger" ? "border-white" : "border-gray-400 dark:border-zinc-600"}`}>
          ...
          <p className="text-sm text-gray-800 dark:text-zinc-200">Facebook Messenger</p>
        </label>
        <div className="px-4 py-3 opacity-50">
          <div className="flex items-center gap-3 mb-1">
            <div className="w-4 h-4 rounded-full border-2 border-gray-300 dark:border-zinc-700 shrink-0" />
            <p className="text-sm text-gray-500 dark:text-zinc-400">Instagram DM</p>
          </div>
          <p className="text-[11px] text-gray-500 dark:text-zinc-500 ml-7">Selected ad account doesn&apos;t have Instagram connected</p>
          <p className="text-[11px] text-gray-400 dark:text-zinc-600 ml-7 mt-0.5">Instagram DMs are disabled because the selected advertising account does not have Instagram logged in.</p>
        </div>
      </div>
    </div>
  );
...
function MetaPickerModal({
  ...
  const content = (
    <div className="fixed inset-0 z-[999999] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="w-full max-w-lg bg-white dark:bg-zinc-950 border border-gray-200 dark:border-zinc-800 rounded-2xl shadow-2xl flex flex-col overflow-hidden" style={{ maxHeight: "84vh" }}>

        {/* Header */}
        <div className="flex items-center justify-between px-6 py-5 shrink-0">
          <h3 className="text-lg font-bold text-gray-900 dark:text-white">Which Facebook ad accounts do you want to add?</h3>
          <button onClick={onClose} className="p-1.5 text-gray-500 dark:text-zinc-500 hover:text-white hover:bg-gray-200 dark:bg-zinc-800 rounded-lg transition-colors ml-4 shrink-0">
            <X className="w-4 h-4" />
          </button>
        </div>

        {/* Body */}
        <div className="flex-1 overflow-y-auto border-t border-gray-200 dark:border-zinc-800/60">
          {loading ? (
            <div className="flex items-center justify-center py-16 gap-3">
              <Loader2 className="w-5 h-5 animate-spin text-gray-500 dark:text-zinc-500" />
              <span className="text-sm text-gray-500 dark:text-zinc-500">Loading from Metaâ€¦</span>
            </div>
          ) : fetchErr ? (
            <div className="p-6 text-center space-y-2">
              <p className="text-sm text-rose-400">{fetchErr}</p>
              <p className="text-xs text-gray-400 dark:text-zinc-600">Token may be expired. Reconnect from Platform Accounts.</p>
            </div>
          ) : (
            <>
              {/* Ad Accounts list */}
              {adAccounts.length === 0 ? (
                <div className="px-6 py-8 text-center">
                  <p className="text-sm text-gray-500 dark:text-zinc-400">No ad accounts found under this Facebook account.</p>
                  <p className="text-xs text-gray-400 dark:text-zinc-600 mt-1">Create a Meta Business Manager account to get started.</p>
                </div>
              ) : (
                    ...
                    return (
                      <div key={a.id}>
                        <button type="button"
                          ...
                          className={`w-full flex items-start gap-4 px-6 py-4 text-left transition-colors hover:bg-white/60 dark:bg-zinc-900/60 cursor-pointer ${
                            isSelected ? "bg-gray-50 dark:bg-zinc-900/40" : ""
                          }`}>
                          {/* Checkbox */}
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[11px] text-gray-500 dark:text-zinc-500 mt-0.5">id: {a.id.replace("act_", "")}</p>
                            <p className="text-[10px] text-gray-400 dark:text-zinc-600">{a.currency} Â· ${a.amount_spent} spent</p>
                          </div>
                        </button>
              ...
              {pages.length > 0 && (
                <div className="border-t border-gray-200 dark:border-zinc-800/60">
                  <p className="px-6 pt-4 pb-2 text-[10px] font-bold text-gray-500 dark:text-zinc-500 uppercase tracking-widest">Which Facebook page to use for ads?</p>
                  <div className="divide-y divide-gray-200 dark:divide-zinc-800/40">
                    {pages.map(p => {
                      const isSelected = selPage === p.id;
                      return (
                        <button key={p.id} type="button" onClick={() => setSelPage(isSelected ? "" : p.id)}
                          className="w-full flex items-center gap-4 px-6 py-3.5 text-left hover:bg-white/60 dark:bg-zinc-900/60 transition-colors">
                          ...
                          {p.picture ? (
                            <Image src={p.picture} alt="" width={32} height={32} className="rounded-full shrink-0 border border-gray-300 dark:border-zinc-700 object-cover" unoptimized />
                          ) : (
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[11px] text-gray-500 dark:text-zinc-500">Facebook Page</p>
                          </div>
                        </button>
                      );
                    })}
        ...
        {/* Footer */}
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 shrink-0">
          <button onClick={onClose} className="px-5 py-2 text-sm text-gray-500 dark:text-zinc-400 hover:text-white transition-colors">
            Cancel
          </button>
          <button
            onClick={handleDone}
            disabled={!selAdAcct || saving || loading}
            className="px-6 py-2 bg-gray-50 dark:bg-zinc-900 hover:bg-gray-200 dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 hover:border-gray-400 dark:hover:border-zinc-500 text-gray-900 dark:text-white text-sm font-bold rounded-lg transition-colors disabled:opacity-40 min-w-[80px]">
...
export default function CampaignCreatorModal({ onClose, onCreated, editId }: {
  ...
  const modal = createPortal(
    <div className="fixed inset-0 z-[99999] flex items-start justify-center bg-black/65 backdrop-blur-sm pt-3 px-4">
      <div className="relative w-full bg-gray-50 dark:bg-zinc-900 rounded-xl shadow-2xl flex flex-col overflow-hidden" style={{ maxWidth: 1100, height: "96vh" }}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-zinc-800 shrink-0">
          <h2 className="text-base font-bold text-gray-900 dark:text-white">{editId ? "Edit Facebook ad campaign" : "New Facebook ad campaign"}</h2>
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className={`shrink-0 border-r border-gray-200 dark:border-zinc-800 flex flex-col transition-all duration-200 ${modalSidebarOpen ? "w-52" : "w-8"}`}>
            {/* Collapse toggle */}
            <button type="button" onClick={() => setModalSidebarOpen(o => !o)}
              className="flex items-center justify-center w-full h-8 text-gray-400 dark:text-zinc-600 hover:text-gray-500 dark:text-zinc-400 hover:bg-gray-100 dark:bg-zinc-800/50 transition-colors shrink-0">
              ...
              return (
                <div key={s.num}
                  className={`px-4 py-3 border-l-2 transition-colors ${
                    ...
                    done    ? "border-gray-300 dark:border-zinc-700 cursor-pointer hover:bg-gray-100 dark:bg-zinc-800/50" :
                ...
                >
                  <div className="flex items-center gap-2 mb-1">
                    <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 ${
                      ...
                      done    ? "bg-zinc-600 text-gray-900 dark:text-white" : "bg-gray-200 dark:bg-zinc-800 text-gray-500 dark:text-zinc-500"
                    }`}>{s.num}</div>
                    <p className={`text-xs font-bold ${current ? "text-gray-900 dark:text-white" : "text-gray-500 dark:text-zinc-400"}`}>{s.label}</p>
                  </div>
                  {(current || done) && sums.map((line, i) => (
                    <p key={i} className="text-[10px] text-gray-500 dark:text-zinc-500 pl-7 leading-snug">{line}</p>
                  ))}
                </div>
              );
            })}
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {step === 1 && (
              <div className="p-6 space-y-6">
                <div className="space-y-4">
                  <div>
                    <h4 className="text-sm font-bold text-gray-900 dark:text-white">Facebook Page and ad account</h4>
                    <p className="text-xs text-gray-500 dark:text-zinc-500 mt-0.5">Choose the Facebook Page and ad account to use for this campaign.</p>
                  </div>
                  ...
                  {adAccounts.length === 0 ? (
                    ...
                    <div className="p-4 bg-amber-500/5 border border-amber-500/20 rounded-xl space-y-2">
                      ...
                      <p className="text-xs text-gray-500 dark:text-zinc-500">Connect a Facebook account first to run ad campaigns.</p>
                      <a href="/accounts" target="_blank"
                        className="inline-flex items-center gap-1.5 mt-1 px-3 py-1.5 bg-[#1877F2] hover:bg-[#1877F2]/90 text-gray-900 dark:text-white text-xs font-semibold rounded-lg transition-colors">
                    ...
                    <>
                      ...
                      {(() => {
                        ...
                        return (
                          <div className="border border-gray-300 dark:border-zinc-700 rounded-xl overflow-hidden">
                            {/* Facebook account row */}
                            <div className="flex items-center justify-between gap-3 px-4 py-3 bg-gray-50 dark:bg-zinc-900/40">
                              <div className="flex items-center gap-3 min-w-0">
                                ...
                                <div className="min-w-0">
                                  <p className="text-sm font-semibold text-gray-900 dark:text-white truncate">{sel?.account_name || "Facebook"}</p>
                                  {adAccounts.length > 1 && (
                                    <div className="relative mt-0.5">
                                      <select value={selectedAcc} onChange={e => setAcc(e.target.value)}
                                        className="text-[10px] text-gray-500 dark:text-zinc-500 bg-transparent outline-none cursor-pointer pr-3 appearance-none">
                                        {adAccounts.map(a => (
                            ...
                            <div className="divide-y divide-gray-200 dark:divide-zinc-800/60">
                              <div className="flex items-center justify-between px-4 py-2.5">
                                <div className="flex items-center gap-2">
                                  <p className="text-[11px] text-gray-500 dark:text-zinc-500 w-20 shrink-0">Ad Account</p>
                                  {hasAdAccount ? (
                              ...
                              <div className="flex items-center gap-2 px-4 py-2.5">
                                <p className="text-[11px] text-gray-500 dark:text-zinc-500 w-20 shrink-0">Page</p>
                                <span className="text-[11px] text-gray-700 dark:text-zinc-300 truncate">{sel?.account_name || "â€”"}</span>
                              </div>
                              {/* Meta ad account balance â€” fetched from Meta Graph API */}
                              {metaBalance && (
                                <div className="px-4 py-2.5 flex items-center gap-4 bg-white/60 dark:bg-zinc-900/60 border-t border-gray-200 dark:border-zinc-800/60">
                                  <div>
                                    <p className="text-[10px] text-gray-400 dark:text-zinc-600 uppercase tracking-wider mb-0.5">Account Balance</p>
                                    <p className={`text-xs font-bold ${metaBalance.balance !== null ? "text-emerald-400" : "text-gray-500 dark:text-zinc-500"}`}>
                                  ...
                                  <div>
                                    <p className="text-[10px] text-gray-400 dark:text-zinc-600 uppercase tracking-wider mb-0.5">Spent</p>
                                  ...
                                  {metaBalance.spend_cap && (
                                    <div>
                                      <p className="text-[10px] text-gray-400 dark:text-zinc-600 uppercase tracking-wider mb-0.5">Spend Cap</p>
                                      <p className="text-xs font-semibold text-gray-500 dark:text-zinc-400">
                                        {metaBalance.currency} {parseFloat(metaBalance.spend_cap).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                                      </p>
                                    </div>
                                  )}
                                  <p className="text-[10px] text-gray-400 dark:text-zinc-600 ml-auto">Live from Meta</p>
                                </div>
                              )}
                            </div>
                          </div>
                      ...
                      <a href="/accounts" target="_blank"
                        className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-zinc-500 hover:text-gray-700 dark:text-zinc-300 transition-colors">
                        <span className="w-3.5 h-3.5 rounded-full border border-gray-400 dark:border-zinc-600 flex items-center justify-center text-[9px]">+</span>
                ...
                <div className="border-t border-gray-200 dark:border-zinc-800 pt-5 space-y-5">
                ...
                <div className="border-t border-gray-200 dark:border-zinc-800 pt-5 space-y-4">
                  <div>
                    <h4 className="text-sm font-bold text-gray-900 dark:text-white mb-0.5">Campaign objective</h4>
                    <p className="text-xs text-gray-500 dark:text-zinc-500">Your campaign objective is the business outcome you want to achieve with your campaign.</p>
                  </div>
                  <p className="text-xs font-semibold text-gray-500 dark:text-zinc-400">Select a campaign objective</p>
                ...
                <div className="border-t border-gray-200 dark:border-zinc-800 pt-5 space-y-4">
                  <div>
                    <h4 className="text-sm font-bold text-gray-900 dark:text-white mb-0.5">Conversion location</h4>
                    <p className="text-xs text-gray-500 dark:text-zinc-500">Choose where you want people to take your desired action.</p>
                  </div>
                    ...
                    <Field label="Select a type of engagement">
                      <div className="relative">
                        <button type="button" onClick={() => setEngTypeOpen(o => !o)}
                          ...
                          <ChevronDown className={`w-4 h-4 text-gray-500 dark:text-zinc-500 transition-transform ${engTypeOpen ? "rotate-180" : ""}`} />
                        </button>
                        {engTypeOpen && (
                          <div className="absolute top-full left-0 right-0 mt-1 z-50 bg-gray-50 dark:bg-zinc-900 border border-gray-300 dark:border-zinc-700 rounded-xl shadow-2xl overflow-hidden">
                            {[
                              ...
                              <button key={et.value} type="button"
                                ...
                                className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-colors ${
                                  engType === et.value ? "bg-amber-500/10 text-gray-900 dark:text-white" : "hover:bg-gray-200 dark:bg-zinc-800 text-gray-700 dark:text-zinc-300"
                                }`}>
                ...
                <div className="border-t border-gray-200 dark:border-zinc-800 pt-5 space-y-3">
                  <div>
                    <h4 className="text-sm font-bold text-gray-900 dark:text-white mb-0.5">Optimization for ad delivery</h4>
                    <p className="text-xs text-gray-500 dark:text-zinc-500">Choose a key result you want Meta to optimize for. Your choice affects who will see your ads.</p>
                  </div>
                  <Field label="Optimize for" hint="The optimization event you choose is the outcome that you want Meta to get you as efficiently as possible. For example, if you choose to optimize for link clicks, Meta shows your ad to people who are most likely to click your link.">
                    <div className="relative">
                      ...
                      <button type="button" onClick={() => setOptimizeOpen(o => !o)}
                        ...
                        <ChevronDown className={`w-4 h-4 text-gray-500 dark:text-zinc-500 transition-transform ${optimizeOpen ? "rotate-180" : ""}`} />
                      </button>
                      {/* Dropdown */}
                      {optimizeOpen && (
                        <div className="absolute top-full left-0 right-0 mt-1 z-50 bg-gray-50 dark:bg-zinc-900 border border-gray-300 dark:border-zinc-700 rounded-xl shadow-2xl overflow-hidden">
                          {(OPTIMIZE_OPTIONS[effectiveObjective] || []).map(o => (
                            <button key={o.value} type="button"
                              ...
                              className={`w-full flex items-start gap-3 px-4 py-3.5 text-left transition-colors border-b border-gray-200 dark:border-zinc-800/50 last:border-0 ${
                                optimize === o.value ? "bg-amber-500/10" : "hover:bg-gray-200 dark:bg-zinc-800"
                              }`}>
                              <div className="mt-0.5 shrink-0">
                                {optimize === o.value ? (
                                  ...
                                  <div className="w-4 h-4 rounded-full border-2 border-gray-400 dark:border-zinc-600" />
                                )}
                              </div>
                              <div>
                                <p className="text-sm font-semibold text-gray-900 dark:text-white">{o.label}</p>
                                <p className="text-xs text-gray-500 dark:text-zinc-500 mt-0.5 leading-relaxed">{o.desc}</p>
                              </div>
                            </button>
                          ))}
                        </div>
                ...
                <div className="border-t border-gray-200 dark:border-zinc-800 pt-5 space-y-4">
                  <div>
                    <p className="text-sm font-bold text-gray-900 dark:text-white mb-0.5">Is your ad in a special category?</p>
                    <p className="text-xs text-gray-500 dark:text-zinc-500">If you are based in or targeting an audience in the United States and your ad relates to credit, employment, or housing, you must identify it to comply with Meta&apos;s advertising policies. Special ad categories have restricted targeting options. <span className="text-gray-500 dark:text-zinc-400 underline cursor-pointer">Learn more</span></p>
                  </div>
                  ...
                  <label className="flex items-start gap-3 cursor-pointer group">
                    ...
                    <p className="text-sm text-gray-700 dark:text-zinc-300 group-hover:text-white transition-colors">
                  ...
                  {specialCat && (
                    <div className="space-y-3 ml-7">
                      ...
                      <div className="relative">
                        ...
                        <ChevronDown className="w-4 h-4 text-gray-500 dark:text-zinc-500 absolute right-3 top-2.5 pointer-events-none" />
                      </div>

                      {/* Important info box */}
                      <div className="flex items-start gap-3 p-4 bg-blue-500/5 border border-blue-500/20 rounded-xl">
                        <div className="w-5 h-5 rounded-full bg-blue-500 flex items-center justify-center text-gray-900 dark:text-white text-[10px] font-bold shrink-0 mt-0.5">i</div>
                        <div className="space-y-2">
                          <p className="text-sm font-bold text-gray-900 dark:text-white">Important information about special ad categories</p>
                          <p className="text-xs text-gray-500 dark:text-zinc-400">To help you comply with Facebook&apos;s advertising policies, some audience targeting options are restricted for ads in special categories.</p>
                          <ul className="space-y-1.5 mt-2">
                            {[
                              ...
                              <li key={label} className="flex items-start gap-1.5 text-xs text-gray-500 dark:text-zinc-400">
                                <span className="text-gray-700 dark:text-zinc-300 font-semibold shrink-0">{label}</span>
                                <span>{value}</span>
                              </li>
                            ))}
                          </ul>
            ...
            {step === 2 && (
              <div className="p-6 space-y-6">
                <div>
                  <h3 className="text-base font-bold text-gray-900 dark:text-white mb-1">Decide on your audience and budget</h3>
                  <p className="text-sm text-gray-500 dark:text-zinc-500">Choose who you want to see your ad on Facebook, then set your budget and when you want your campaign to run.</p>
                </div>

                {/* Audience */}
                <div className="border-t border-gray-200 dark:border-zinc-800 pt-5 space-y-4">
                  <div>
                    <h4 className="text-sm font-bold text-gray-900 dark:text-white mb-0.5">Audience</h4>
                    <p className="text-xs text-gray-500 dark:text-zinc-500">Your audience is the group of people who will potentially see your ad. Use our default audience settings or an audience you created on Facebook.</p>
                  </div>
                  ...
                  <Field label="Select an audience">
                    <div className="relative">
                      <button type="button" onClick={() => setAudDropOpen(o => !o)}
                        className={inp + " flex items-center justify-between text-left"}>
                        <span className="text-gray-900 dark:text-white font-medium">Build your own audience</span>
                        <ChevronDown className={`w-4 h-4 text-gray-500 dark:text-zinc-500 transition-transform ${audDropOpen ? "rotate-180" : ""}`} />
                      </button>
                      ...
                      {audDropOpen && (
                        <div className="absolute top-full left-0 right-0 mt-1 z-50 bg-gray-50 dark:bg-zinc-900 border border-gray-300 dark:border-zinc-700 rounded-xl shadow-2xl overflow-hidden">
                          ...
                          <button type="button" onClick={() => setAudDropOpen(false)}
                            className="w-full flex items-start gap-3 px-4 py-3.5 text-left bg-amber-500/5 border-b border-gray-200 dark:border-zinc-800 hover:bg-amber-500/10 transition-colors">
                            <span className="text-amber-400 text-xs mt-0.5 shrink-0">âœ“</span>
                            <div>
                              <p className="text-sm font-bold text-gray-900 dark:text-white">Build your own audience</p>
                              <p className="text-[11px] text-gray-500 dark:text-zinc-400 mt-0.5 leading-relaxed">Choose who you want to see your ad based on targeting settings including their location, age, interests, and others.</p>
                            </div>
                          </button>

                          {/* Use saved audience */}
                          <div className="px-4 py-3.5 border-b border-gray-200 dark:border-zinc-800/50 opacity-60">
                            <div className="flex items-center justify-between mb-1">
                              <p className="text-xs font-semibold text-gray-500 dark:text-zinc-400">Use saved audience</p>
                              <span className="text-[10px] font-bold px-2 py-0.5 bg-gray-300 dark:bg-zinc-700 text-gray-500 dark:text-zinc-400 rounded-full shrink-0 ml-2">NO EXISTING AUDIENCE</span>
                            </div>
                            <p className="text-[11px] text-gray-400 dark:text-zinc-600 leading-relaxed">Choose a saved audience to reach people based on targeting options you&apos;ve already set on Facebook.</p>
                          </div>

                          {/* Use custom or lookalike */}
                          <div className="px-4 py-3.5 opacity-60">
                            <div className="flex items-center justify-between mb-1">
                              <p className="text-xs font-semibold text-gray-500 dark:text-zinc-400">Use custom or lookalike audiences</p>
                              <span className="text-[10px] font-bold px-2 py-0.5 bg-gray-300 dark:bg-zinc-700 text-gray-500 dark:text-zinc-400 rounded-full shrink-0 ml-2">NO EXISTING AUDIENCE</span>
                            </div>
                            <p className="text-[11px] text-gray-400 dark:text-zinc-600 leading-relaxed">Choose custom and lookalike audiences you&apos;ve created on Facebook to reach people who have already engaged with your brand, or to reach new people who are similar to the audiences you&apos;re already targeting.</p>
                          </div>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="bg-gray-200 dark:bg-zinc-800/40 border border-gray-300 dark:border-zinc-700 rounded-xl p-4 space-y-2">
                    <p className="text-[10px] font-bold text-gray-500 dark:text-zinc-500 uppercase tracking-widest">Included people who match</p>
                    <p className="text-xs text-gray-700 dark:text-zinc-300">Age: {ageMin} - {ageMax}</p>
                    <p className="text-xs text-gray-700 dark:text-zinc-300">Gender: {gender === "ALL" ? "All" : gender.charAt(0) + gender.slice(1).toLowerCase()}</p>
                    <p className="text-xs text-gray-700 dark:text-zinc-300">Location: {(() => { try { return (JSON.parse(location) as {display_name:string}[]).map(l => l.display_name).join(", "); } catch { return location || "â€”"; } })()}</p>
                    {interests.length > 0 && <p className="text-xs text-gray-700 dark:text-zinc-300">Interests: {interests.map(i => i.name).join(", ")}</p>}
                    ...
                    <div className="pt-2 border-t border-gray-300 dark:border-zinc-700/50 flex items-center justify-between">
                      <div>
                        <p className="text-[10px] text-gray-500 dark:text-zinc-500">Potential reach</p>
                        <p className="text-lg font-bold text-gray-900 dark:text-white">{reachLoading ? <span className="text-gray-500 dark:text-zinc-500 text-sm">Estimating...</span> : potentialReach}</p>
                      </div>
                      <button type="button"
                        onClick={() => {
                        ...
                        className="px-3 py-1.5 text-xs font-semibold text-gray-900 dark:text-white border border-gray-400 dark:border-zinc-600 hover:border-zinc-400 rounded-lg transition-colors">
                        Edit audience
                      </button>
                    </div>
                  </div>
                ...
                {showEditAud && (
                  <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/70">
                    <div className="w-full max-w-2xl bg-white dark:bg-zinc-950 border border-gray-200 dark:border-zinc-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col" style={{ maxHeight: "85vh" }}>
                      {/* Header */}
                      <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-zinc-800">
                        <h3 className="text-base font-bold text-gray-900 dark:text-white">Edit audience</h3>
                        <button onClick={() => setShowEditAud(false)} className="p-1.5 text-gray-500 dark:text-zinc-500 hover:text-white hover:bg-gray-200 dark:bg-zinc-800 rounded-lg transition-colors">
                          <X className="w-4 h-4" />
                        </button>
                      </div>
                      <div className="flex flex-1 overflow-hidden">
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-6">
                          ...
                          <div className="space-y-3">
                            ...
                            <p className="text-xs text-gray-500 dark:text-zinc-500">Target people by including or excluding their location</p>

                            {/* Include */}
                            <p className="text-xs font-semibold text-gray-500 dark:text-zinc-400">Include</p>
                            <div className="border border-gray-300 dark:border-zinc-700 rounded-lg p-2 bg-gray-50 dark:bg-zinc-900">
                              <div className="flex flex-wrap gap-1.5 mb-2">
                                {tmpLoc.map(l => (
                                  <span key={l.key} className="flex items-center gap-1 text-xs bg-gray-200 dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 rounded px-2 py-1 text-gray-700 dark:text-zinc-300">
                                    <span className="w-2 h-2 rounded-full bg-zinc-500 inline-block" />
                                    {l.display_name}
                                    <button onClick={() => setTmpLoc(tmpLoc.filter(x => x.key !== l.key))} className="text-gray-500 dark:text-zinc-500 hover:text-rose-400 ml-0.5">
                                      <X className="w-3 h-3" />
                                    </button>
                                  </span>
                                ))}
                                <div className="relative flex-1 min-w-32">
                                  <input
                                    ...
                                    className="w-full text-sm outline-none placeholder:text-gray-400 dark:text-zinc-600 text-gray-900 dark:text-white bg-transparent"
                                  />
                                  {/* Location dropdown */}
                                  {(locResults.length > 0 || locLoading) && locInputVal.length >= 2 && (
                                    <div className="absolute top-full left-0 right-0 mt-1 z-50 bg-gray-200 dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 rounded-lg shadow-xl overflow-hidden max-h-48 overflow-y-auto min-w-56">
                                      {locLoading && <p className="px-3 py-2 text-xs text-gray-500 dark:text-zinc-500">Searching...</p>}
                                      {locResults.map(l => (
                                        <button key={l.key} type="button"
                                          ...
                                          className="w-full text-left px-3 py-2 text-sm text-gray-800 dark:text-zinc-200 hover:bg-gray-300 dark:bg-zinc-700 transition-colors border-b border-gray-300 dark:border-zinc-700/50 last:border-0">
                                          {l.display_name}
                                        </button>
                                      ))}
                                    </div>
                            ...
                            <p className="text-[11px] text-gray-400 dark:text-zinc-600">Locations powered by Meta Targeting API â€” requires connected Facebook account with ads_management scope.</p>
                            ...
                            {showExclude && (
                              <div className="space-y-2">
                                <p className="text-xs font-semibold text-gray-500 dark:text-zinc-400">Exclude</p>
                                <div className="border border-gray-300 dark:border-zinc-700 rounded-lg p-2 bg-gray-50 dark:bg-zinc-900">
                                  <div className="flex flex-wrap gap-1.5 mb-2">
                                    ...
                                    <div className="relative flex-1 min-w-32">
                                      ...
                                      {(exLocResults.length > 0 || exLocLoading) && exLocInput.length >= 2 && (
                                        <div className="absolute top-full left-0 right-0 mt-1 z-50 bg-gray-200 dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 rounded-lg shadow-xl overflow-hidden max-h-40 overflow-y-auto min-w-56">
                                          {exLocLoading && <p className="px-3 py-2 text-xs text-gray-500 dark:text-zinc-500">Searching...</p>}
                                          {exLocResults.map(l => (
                                            <button key={l.key} type="button"
                                              onClick={() => {
                                              ...
                                              className="w-full text-left px-3 py-2 text-sm text-rose-200 hover:bg-gray-300 dark:bg-zinc-700 transition-colors border-b border-gray-300 dark:border-zinc-700/50 last:border-0">
                                              {l.display_name}
                                            </button>
                                          ))}
                                        </div>
                          ...
                          <div className="space-y-3">
                            <div className="flex items-center justify-between">
                              <p className="text-sm font-bold text-gray-900 dark:text-white">Gender</p>
                              {specialCat && specialCatType && <span className="text-[10px] text-amber-400 font-semibold">Locked by special category</span>}
                            </div>
                            <div className={`flex gap-0 border rounded-lg overflow-hidden w-fit ${specialCat && specialCatType ? "border-gray-200 dark:border-zinc-800 opacity-50 pointer-events-none" : "border-gray-300 dark:border-zinc-700"}`}>
                              {["ALL","FEMALE","MALE"].map((g, i) => (
                                <button key={g} type="button" onClick={() => !specialCat && setTmpGender(g)}
                                  disabled={!!(specialCat && specialCatType)}
                                  className={`px-6 py-2 text-sm font-medium transition-all ${i > 0 ? "border-l border-gray-300 dark:border-zinc-700" : ""} ${
                                    tmpGender === g ? "bg-white text-zinc-900" : "bg-gray-50 dark:bg-zinc-900 text-gray-500 dark:text-zinc-400 hover:bg-gray-200 dark:bg-zinc-800 hover:text-white"
                                  }`}>
                          ...
                          <div className="space-y-3">
                            <div className="flex items-center justify-between">
                              <p className="text-sm font-bold text-gray-900 dark:text-white">Age</p>
                              {specialCat && specialCatType && <span className="text-[10px] text-amber-400 font-semibold">Locked: 18â€“65+ by special category</span>}
                            </div>
                            <p className="text-xs text-gray-500 dark:text-zinc-500">Select the minimum and maximum age for people who will see your ad.<br/>Note: the minimum age is 13, and the maximum age is 65 and over.</p>
                            <div className="flex items-center gap-3">
                              <div className="relative">
                                ...
                                <ChevronDown className="w-4 h-4 text-gray-500 dark:text-zinc-500 absolute right-2 top-2.5 pointer-events-none" />
                              </div>
                              <span className="text-gray-500 dark:text-zinc-500">â€“</span>
                              <div className="relative">
                          ...
                          {/* Detailed targeting */}
                          <div className="space-y-3">
                            <p className="text-sm font-bold text-gray-900 dark:text-white">Detailed targeting</p>
                            ...
                            <div className="border border-gray-300 dark:border-zinc-700 rounded-lg p-2 bg-gray-50 dark:bg-zinc-900">
                              <div className="flex flex-wrap gap-1.5 mb-2">
                                {tmpInt.map(t => (
                                  <span key={t.id} className="flex items-center gap-1 text-xs bg-gray-200 dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 rounded px-2 py-1 text-gray-700 dark:text-zinc-300">
                                    {t.name}
                                    <button onClick={() => setTmpInt(prev => prev.filter(x => x.id !== t.id))} className="text-gray-500 dark:text-zinc-500 hover:text-rose-400 ml-0.5">
                                      <X className="w-3 h-3" />
                                    </button>
                                  </span>
                                ))}
                                <div className="relative flex-1 min-w-40">
                                  ...
                                  {(intResults.length > 0 || intLoading) && intInputVal.length >= 2 && (
                                    <div className="absolute top-full left-0 right-0 mt-1 z-50 bg-gray-200 dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 rounded-lg shadow-xl overflow-hidden max-h-48 overflow-y-auto min-w-56">
                                      {intLoading && <p className="px-3 py-2 text-xs text-gray-500 dark:text-zinc-500">Searching...</p>}
                                      {intResults.map(i => (
                            ...
                            <p className="text-[11px] text-gray-400 dark:text-zinc-600">Search and select interests from Meta&apos;s Targeting API. Requires a connected Facebook account.</p>
                          </div>
                        </div>

                        {/* Right â€” current details */}
                        <div className="w-56 shrink-0 bg-white dark:bg-zinc-900/50 border-l border-gray-200 dark:border-zinc-800 p-5 space-y-4">
                          <p className="text-sm font-bold text-gray-900 dark:text-white">Current audience details</p>
                          <div className="space-y-2">
                            <p className="text-[10px] font-bold text-gray-500 dark:text-zinc-500 uppercase tracking-widest flex items-center gap-1">
                              <span>ðŸ‘¥</span> Included people who match
                            </p>
                            <p className="text-xs text-gray-500 dark:text-zinc-400">Age: {tmpAge[0]} - {tmpAge[1]}</p>
                            <p className="text-xs text-gray-500 dark:text-zinc-400">Gender: {tmpGender === "ALL" ? "All" : tmpGender.charAt(0) + tmpGender.slice(1).toLowerCase()}</p>
                            <p className="text-xs text-gray-500 dark:text-zinc-400">Location: {tmpLoc.map(l => l.display_name).join(", ") || "â€”"}</p>
                            {tmpExcLocItems.length > 0 && <p className="text-xs text-rose-400">Excluded: {tmpExcLocItems.map(l => l.display_name).join(", ")}</p>}
                            {tmpInt.length > 0 && <p className="text-xs text-gray-500 dark:text-zinc-400">Interests: {tmpInt.map(i => i.name).join(", ")}</p>}
                          </div>
                          <div className="pt-3 border-t border-gray-200 dark:border-zinc-800">
                            <p className="text-[10px] font-bold text-gray-500 dark:text-zinc-500 uppercase tracking-widest mb-1">Potential reach</p>
                            <p className="text-xl font-bold text-gray-900 dark:text-white">{potentialReach}</p>
                          </div>
                        </div>
                      </div>

                      {/* Footer */}
                      <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-200 dark:border-zinc-800 bg-white dark:bg-zinc-950">
                        {audienceError && (
                          <p className="text-xs text-rose-400 mr-auto">{audienceError}</p>
                        )}
                        <button onClick={() => { setShowEditAud(false); setAudienceError(null); }}
                          className="px-5 py-2 text-sm text-gray-500 dark:text-zinc-400 border border-gray-300 dark:border-zinc-700 hover:border-gray-400 dark:hover:border-zinc-500 hover:text-white rounded-lg font-medium transition-colors">
                          Cancel
                        </button>
                        <button
                          onClick={() => {
                ...
                <div className="border-t border-gray-200 dark:border-zinc-800 pt-5 space-y-4">
                  <div>
                    <h4 className="text-sm font-bold text-gray-900 dark:text-white mb-1">Do your ads target audiences in the EU?</h4>
                    <p className="text-xs text-gray-500 dark:text-zinc-500">
                      ...
                      <span className="text-gray-500 dark:text-zinc-400 underline cursor-pointer">Learn more about EU requirements</span>
                    </p>
                  </div>
                    ...
                    <div className="ml-7 space-y-4">
                      ...
                      <label className="flex items-start gap-3 cursor-pointer group">
                        <input type="checkbox" checked={euConfirmed} onChange={e => setEuConfirmed(e.target.checked)}
                          className="mt-0.5 w-4 h-4 accent-white rounded shrink-0" />
                        <p className="text-xs text-gray-500 dark:text-zinc-400 group-hover:text-gray-700 dark:text-zinc-300 transition-colors leading-relaxed">
                      ...
                      <div className="space-y-1.5">
                        <div className="flex items-center gap-1.5">
                          <p className="text-sm font-semibold text-gray-800 dark:text-zinc-200">Beneficiary</p>
                          <span className="text-rose-400 text-xs">*</span>
                        </div>
                        <p className="text-xs text-gray-500 dark:text-zinc-500">Provide accurate beneficiary entity name</p>
                        <input
                          value={beneficiary}
                      ...
                      <div className="space-y-1.5">
                        <div className="flex items-center gap-1.5">
                          <p className="text-sm font-semibold text-gray-800 dark:text-zinc-200">Payer</p>
                          <span className="text-rose-400 text-xs">*</span>
                        </div>
                        <p className="text-xs text-gray-500 dark:text-zinc-500">Provide accurate payer entity name</p>
                        <input
                          value={payer}
                ...
                <div className="border-t border-gray-200 dark:border-zinc-800 pt-5 space-y-4">
                  <div>
                    <h4 className="text-sm font-bold text-gray-900 dark:text-white mb-0.5">Placements</h4>
                    <p className="text-xs text-gray-500 dark:text-zinc-500">
                      ...
                      <span className="text-gray-500 dark:text-zinc-400 underline cursor-pointer">Learn more about placements</span>
                    </p>
                  </div>
                  ...
                  <label className="flex items-center gap-3 cursor-pointer">
                    ...
                    <p className="text-sm text-gray-700 dark:text-zinc-300">I want to manually choose the placements</p>
                  </label>

                  {/* Automatic placements info */}
                  {autoPlace && (
                    <div className="bg-gray-100 dark:bg-zinc-800/50 border border-gray-300 dark:border-zinc-700 rounded-xl p-3">
                      <p className="text-xs font-semibold text-gray-700 dark:text-zinc-300">Automatic placements</p>
                      <p className="text-xs text-gray-500 dark:text-zinc-500 mt-0.5">Facebook, Instagram, Messenger, Audience Network</p>
                    </div>
                  )}
                  ...
                  {!autoPlace && (() => {
                    ...
                    return (
                      <div className="space-y-3">
                        {/* Device selector */}
                        <div>
                          <p className="text-xs font-semibold text-gray-500 dark:text-zinc-400 mb-1.5">Select device</p>
                        ...
                        <div className="flex items-center justify-between">
                          <p className="text-sm font-semibold text-gray-800 dark:text-zinc-200">
                            Selected placements <span className="text-gray-500 dark:text-zinc-500">({selCount} / {total})</span>
                          </p>
                        </div>

                        {/* Placement rows */}
                        <div className="border border-gray-300 dark:border-zinc-700 rounded-xl overflow-hidden divide-y divide-gray-200 dark:divide-zinc-800/60 max-h-72 overflow-y-auto">
                          ...
                          {deviceFiltered.map(p => (
                            <div key={p.id} className="flex items-center gap-3 px-4 py-3">
                              ...
                              <div className="flex-1 min-w-0">
                                <p className="text-sm text-gray-800 dark:text-zinc-200">{p.label}</p>
                              </div>
                              <span className={`text-[10px] font-semibold shrink-0 ${PLATFORM_COLOR[p.platform] || "text-gray-500 dark:text-zinc-500"}`}>
                                {p.platform}
                              </span>
                            </div>
                          ))}
                          ...
                          {deviceBlocked.map(p => (
                            <div key={p.id} className="flex items-center gap-3 px-4 py-3 opacity-40">
                              ...
                              <div className="flex-1 min-w-0">
                                <p className="text-sm text-gray-500 dark:text-zinc-400">{p.label}</p>
                          ...
                          {objUnavailable.map(p => (
                            <div key={p.id} className="flex items-center gap-3 px-4 py-3 opacity-35">
                              <PlacementToggle id={p.id} disabled />
                              <div className="flex-1 min-w-0">
                                <p className="text-sm text-gray-500 dark:text-zinc-500">{p.label}</p>
                                <p className="text-[11px] text-gray-400 dark:text-zinc-600 mt-0.5">
                ...
                {objective === "CONVERSIONS" && convLocation !== "message" && optimize === "OFFSITE_CONVERSIONS" && (() => {
                  ...
                  return (
                    <div className="border-t border-gray-200 dark:border-zinc-800 pt-5 space-y-4">
                      <div>
                        <h4 className="text-sm font-bold text-gray-900 dark:text-white mb-0.5">Tracking</h4>
                        <p className="text-xs text-gray-500 dark:text-zinc-500">
                          ...
                          <span className="text-gray-500 dark:text-zinc-400 underline cursor-pointer">Learn more about conversion tracking</span>
                        </p>
                      </div>
                      ...
                      <Field label="Select a conversion event">
                        <div className="relative">
                          <button type="button"
                            ...
                            <span className={convEvent ? "text-gray-900 dark:text-white" : "text-gray-400 dark:text-zinc-600"}>
                              {convEvent || "Search for a conversion event"}
                            </span>
                            <ChevronDown className={`w-4 h-4 text-gray-500 dark:text-zinc-500 transition-transform ${eventDropOpen ? "rotate-180" : ""}`} />
                          </button>
                          {eventDropOpen && (
                            <div className="absolute top-full left-0 right-0 mt-1 z-50 bg-gray-50 dark:bg-zinc-900 border border-gray-300 dark:border-zinc-700 rounded-xl shadow-2xl overflow-hidden">
                              ...
                              <div className="max-h-56 overflow-y-auto">
                                {filteredEvents.map(e => (
                                  <button key={e} type="button"
                                    ...
                                    className="w-full text-left px-4 py-2.5 text-sm text-gray-800 dark:text-zinc-200 hover:bg-gray-200 dark:bg-zinc-800 transition-colors border-b border-gray-200 dark:border-zinc-800/30 last:border-0">
                                    {e}
                                  </button>
                                ))}
                              </div>
                        ...
                        {!trackingPixel && <p className="text-[11px] text-gray-400 dark:text-zinc-600 mt-1">Select a pixel first.</p>}
                      </Field>
                    </div>
                  );
                })()}
                ...
                <div className="border-t border-gray-200 dark:border-zinc-800 pt-5 space-y-4">
                  <div>
                    <h4 className="text-sm font-bold text-gray-900 dark:text-white mb-0.5">Budget and duration</h4>
                    <p className="text-xs text-gray-500 dark:text-zinc-500">Set the budget and duration of your ad campaign.</p>
                  </div>

                  {/* Meta ad account live balance */}
                  {metaBalance ? (
                    <div className="flex items-center gap-5 px-4 py-3 bg-gray-50 dark:bg-zinc-900 border border-gray-300 dark:border-zinc-700/60 rounded-xl">
                      <div className="flex-1">
                        <p className="text-[10px] font-bold text-gray-500 dark:text-zinc-500 uppercase tracking-wider mb-0.5">Account Balance</p>
                        <p className={`text-base font-bold ${metaBalance.balance !== null ? "text-emerald-400" : "text-gray-500 dark:text-zinc-500"}`}>
                          {metaBalance.balance !== null
                      ...
                      <div className="flex-1">
                        <p className="text-[10px] font-bold text-gray-500 dark:text-zinc-500 uppercase tracking-wider mb-0.5">Total Spent</p>
                      ...
                      {metaBalance.spend_cap && (
                        <>
                          <div className="w-px h-8 bg-gray-200 dark:bg-zinc-800" />
                          <div className="flex-1">
                            <p className="text-[10px] font-bold text-gray-500 dark:text-zinc-500 uppercase tracking-wider mb-0.5">Spend Cap</p>
                            <p className="text-base font-semibold text-gray-500 dark:text-zinc-400">
                              {symbol}{parseFloat(metaBalance.spend_cap).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            </p>
                          </div>
                        </>
                      )}
                      <p className="text-[10px] text-gray-400 dark:text-zinc-600 shrink-0">Live Â· Meta</p>
                    </div>
                  ) : (
                    <div className="h-[60px] flex items-center px-4 bg-white dark:bg-zinc-900/50 border border-gray-200 dark:border-zinc-800 rounded-xl">
                      <p className="text-xs text-gray-400 dark:text-zinc-600">Connecting to Meta ad accountâ€¦</p>
                    </div>
                  )}
                  ...
                  <Field label="Budget" hint={`Enter in ${currency}. Meta minimum is ${symbol}${minAmt}/day for this ad account.`}>
                    <div className="flex items-center gap-4">
                      <div className="relative w-36">
                        <span className="absolute left-3 top-2.5 text-gray-500 dark:text-zinc-400 text-sm">{symbol}</span>
                      ...
                      <div className="flex gap-3">
                        {[{v:"daily",l:"Per day"},{v:"total",l:"Total"}].map(({v,l}) => (
                          <label key={v} className="flex items-center gap-1.5 cursor-pointer">
                            <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${budgetType === v ? "border-white" : "border-gray-400 dark:border-zinc-600"}`}>
                            ...
                            <span className="text-sm text-gray-700 dark:text-zinc-300">{l}</span>
                          </label>
                        ))}
                      </div>
                    </div>
                  ...
                  <Field label="Duration" hint="Choose how long your ad campaign will run.">
                    <div className="flex items-center gap-3">
                      <div className="flex-1">
                        <p className="text-[10px] text-gray-500 dark:text-zinc-500 mb-1">From</p>
                      ...
                      <div className="flex-1">
                        <p className="text-[10px] text-gray-500 dark:text-zinc-500 mb-1">To</p>
                ...
                <div className="border-t border-gray-200 dark:border-zinc-800 pt-5">
                  <h4 className="text-sm font-bold text-gray-900 dark:text-white mb-1">Payment</h4>
                  <p className="text-xs text-gray-500 dark:text-zinc-500">
            ...
            {step === 3 && (
              <div className="flex h-full overflow-hidden">
                ...
                <div className={`shrink-0 border-r border-gray-200 dark:border-zinc-800 flex flex-col transition-all duration-200 ${adsSidebarOpen ? "w-52" : "w-8"}`}>
                  ...
                  {adsSidebarOpen && (
                    <div className="flex-1 flex flex-col overflow-hidden">
                      {/* Ads List header */}
                      <div className="px-3 pt-1 pb-2 border-b border-gray-200 dark:border-zinc-800">
                        <div className="flex items-center justify-between mb-0.5">
                          <p className="text-xs font-bold text-gray-900 dark:text-white">Ads List</p>
                        </div>
                        <p className="text-[10px] text-gray-500 dark:text-zinc-500 mb-1.5">YOUR ADS ({ads.length}/50)</p>
                        {ads.length < 50 && (
                          <button type="button"
                        ...
                        {ads.map((a, idx) => (
                          <div key={idx}
                            className={`flex items-center gap-2 px-3 py-2 cursor-pointer transition-colors group ${
                              !showSummary && selectedAdIdx === idx ? "bg-amber-500/10" : "hover:bg-gray-200 dark:bg-zinc-800/40"
                            }`}
                            onClick={() => { setSelectedAdIdx(idx); setShowSummary(false); }}>
                            <p className="text-xs text-gray-700 dark:text-zinc-300 flex-1 truncate">{a.name || "New ad"}</p>
                            {!a.image_url && (
                            ...
                            <button type="button"
                              ...
                              className="opacity-0 group-hover:opacity-100 text-gray-400 dark:text-zinc-600 hover:text-rose-400 transition-all shrink-0">
                      ...
                      {/* Campaign / Summary */}
                      <div className="border-t border-gray-200 dark:border-zinc-800 pt-2 pb-1">
                        <p className="px-3 text-[9px] font-bold text-gray-400 dark:text-zinc-600 uppercase tracking-widest mb-1">Campaign</p>
                        <button type="button"
                          onClick={() => setShowSummary(true)}
                          className={`w-full text-left px-3 py-2 text-xs font-medium transition-colors ${showSummary ? "bg-amber-500/10 text-amber-400" : "text-gray-500 dark:text-zinc-400 hover:bg-gray-200 dark:bg-zinc-800/40 hover:text-gray-800 dark:text-zinc-200"}`}>
                          Summary
                        </button>
                      </div>
                    </div>
                ...
                {showSummary ? (
                  ...
                  <div className="flex-1 overflow-y-auto p-6">
                    <h2 className="text-base font-bold text-gray-900 dark:text-white mb-5">Summary</h2>
                    <div className="space-y-0 border border-gray-200 dark:border-zinc-800 rounded-xl overflow-hidden">
                      {[
                        ...
                        <div key={label} className={`flex items-start gap-4 px-5 py-3 ${i % 2 === 0 ? "bg-gray-50 dark:bg-zinc-900/20" : ""} border-b border-gray-200 dark:border-zinc-800/50 last:border-0`}>
                          <p className="text-sm font-semibold text-gray-500 dark:text-zinc-400 w-48 shrink-0">{label}</p>
                ...
                <div className="flex-1 flex flex-col overflow-hidden border-r border-gray-200 dark:border-zinc-800">
                  {/* Ad header row */}
                  <div className="flex items-center gap-3 px-5 py-3 border-b border-gray-200 dark:border-zinc-800 shrink-0">
                    <div className="w-2 h-2 rounded-full bg-amber-500 shrink-0" />
                    <input value={ad.name} onChange={e => setAd(a => ({...a, name: e.target.value}))}
                      className="bg-transparent text-sm font-semibold text-gray-900 dark:text-white outline-none border-b border-transparent hover:border-gray-400 dark:border-zinc-600 focus:border-zinc-400 transition-colors flex-1"
                      placeholder="New ad" />
                  </div>

                  {/* Content tab */}
                  <div className="flex border-b border-gray-200 dark:border-zinc-800 px-5 shrink-0">
                    <button type="button" className="px-1 py-2.5 text-xs font-bold text-gray-900 dark:text-white border-b-2 border-white mr-4">Content</button>
                  </div>
                  ...
                  <div className="flex-1 overflow-y-auto p-5 space-y-4">
                    ...
                    <div className="relative">
                      ...
                      />
                      <p className="text-[10px] text-gray-400 dark:text-zinc-600 text-right mt-1">{ad.copy.length} / 2,200</p>
                    </div>
                            ...
                            <button key={type.value} type="button"
                              ...
                              className={`px-4 py-2 text-sm font-semibold rounded-xl border transition-all ${
                                (ad.ad_type || "image_ad") === type.value
                                  ...
                                  : "bg-gray-50 dark:bg-zinc-900 text-gray-500 dark:text-zinc-400 border-gray-300 dark:border-zinc-700 hover:border-gray-400 dark:hover:border-zinc-500 hover:text-gray-700 dark:text-zinc-300"
                              }`}>
                              {type.label}
                            </button>
                          ))}
                    ...
                    {(ad.ad_type || "image_ad") === "video_ad" && convLocation !== "message" && (
                      <Field label="Facebook Video ID or URL">
                        ...
                        <p className="text-[11px] text-gray-400 dark:text-zinc-600 mt-1.5">Upload your video in Meta Ads Manager â†’ Creative Hub, then paste the video ID here.</p>
                      </Field>
                    )}
                    ...
                    {(ad.ad_type || "image_ad") === "lead_ad" && convLocation !== "message" && (
                      <Field label="Meta Lead Gen Form ID">
                        <input type="text" value={ad.lead_form_id || ""}
                          ...
                          className={inp} placeholder="e.g. 1234567890123456" />
                        <p className="text-[11px] text-gray-400 dark:text-zinc-600 mt-1.5">Create your form in Meta Ads Manager â†’ Lead Ads Forms, then paste the Form ID here.</p>
                      </Field>
                    )}
                    ...
                    {!ad.image_url ? (
                      <div className="space-y-2">
                        ...
                        {/* Upload buttons row */}
                        <div className="flex gap-3">
                          <label className={`flex-1 flex flex-col items-center justify-center gap-2 p-6 border-2 border-dashed rounded-xl transition-colors ${imageUploading ? "border-gray-400 dark:border-zinc-600 cursor-wait" : "border-gray-300 dark:border-zinc-700 hover:border-gray-400 dark:hover:border-zinc-500 cursor-pointer"} group`}>
                            <div className="w-10 h-10 bg-gray-200 dark:bg-zinc-800 group-hover:bg-gray-300 dark:bg-zinc-700 rounded-lg flex items-center justify-center transition-colors">
                              {imageUploading
                                ? <Loader2 className="w-5 h-5 text-gray-500 dark:text-zinc-400 animate-spin" />
                                : <ImageIcon className="w-5 h-5 text-gray-500 dark:text-zinc-400" />}
                            </div>
                            <p className="text-xs text-gray-500 dark:text-zinc-500 group-hover:text-gray-500 dark:text-zinc-400">
                          ...
                          <button type="button" onClick={() => setShowVaultPicker(true)}
                            className="flex-1 flex flex-col items-center justify-center gap-2 p-6 border-2 border-dashed border-gray-300 dark:border-zinc-700 hover:border-gray-400 dark:hover:border-zinc-500 rounded-xl cursor-pointer transition-colors group">
                            <div className="w-10 h-10 bg-gray-200 dark:bg-zinc-800 group-hover:bg-gray-300 dark:bg-zinc-700 rounded-lg flex items-center justify-center transition-colors">
                            ...
                            <p className="text-xs text-gray-500 dark:text-zinc-500 group-hover:text-gray-500 dark:text-zinc-400">Add from library</p>
                          </button>
                        </div>
                        {imageUploadErr && (
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-gray-400 dark:text-zinc-600 mt-1">Must be a direct link (no redirects). Upload using the button above for best results.</p>
                        </div>
                      </div>
                    ) : (
                      <div className="relative rounded-xl overflow-hidden border border-gray-300 dark:border-zinc-700">
                        ...
                        <button type="button" onClick={() => setAd(a => ({...a, image_url: ""}))}
                          className="absolute top-2 right-2 w-6 h-6 bg-black/70 rounded-full flex items-center justify-center text-gray-900 dark:text-white hover:bg-black transition-colors">
                          <X className="w-3 h-3" />
                        </button>
                      </div>
                    )}
                    ...
                    {convLocation === "message" ? (
                      <>
                        {/* Welcome message */}
                        <div className="space-y-1.5">
                          <p className="text-sm font-semibold text-gray-800 dark:text-zinc-200">Welcome message</p>
                          <textarea
                            value={welcomeMsg}
                            ...
                            className={inp + " resize-none text-sm"}
                          />
                          <p className="text-[10px] text-gray-400 dark:text-zinc-600 text-right">{welcomeMsg.length} / 300 characters</p>
                        ...
                        <div className="space-y-2">
                          <label className="flex items-start gap-3 cursor-pointer">
                            <input type="checkbox" checked={addWebsiteUrl} onChange={e => setAddWebsiteUrl(e.target.checked)}
                              className="mt-0.5 w-4 h-4 accent-white rounded shrink-0" />
                            <div>
                              <p className="text-sm font-semibold text-gray-800 dark:text-zinc-200">Add a website URL</p>
                ...
                <div className="flex-1 flex flex-col overflow-hidden">
                    ...
                    {(["desktop","mobile","instagram"] as const).map(t => (
                      <button key={t} onClick={() => setPreview(t)}
                        className={`flex items-center gap-1.5 px-4 py-3 text-sm font-semibold border-b-2 transition-all capitalize ${
                          previewTab === t ? "border-zinc-300 text-gray-900 dark:text-white" : "border-transparent text-gray-500 dark:text-zinc-500 hover:text-gray-700 dark:text-zinc-300"
                        }`}>
                  ...
                  {/* Preview area */}
                  <div className="flex-1 overflow-y-auto flex flex-col items-center justify-start p-5 bg-white dark:bg-zinc-950/50 gap-3">
                    {(() => {
                      ...
                      const imageEl = ad.image_url
                        ...
                        : <div className="w-full bg-zinc-100 flex items-center justify-center" style={{ aspectRatio: previewTab === "instagram" ? "1/1" : "16/9" }}><ImageIcon className="w-8 h-8 text-gray-700 dark:text-zinc-300" /></div>;
                          ...
                          return (
                            <div className="bg-white rounded-xl overflow-hidden shadow-lg w-64">
                              <div className="flex items-center justify-between px-3 py-2 border-b border-zinc-100">
                                <div className="flex items-center gap-2">
                                  ...
                                  <div>
                                    ...
                                    <p className="text-[9px] text-gray-500 dark:text-zinc-500">Sponsored Â· ðŸŒ</p>
                                  </div>
                                </div>
                                <span className="text-gray-500 dark:text-zinc-400">Â·Â·Â·</span>
                              </div>
                              {imageEl}
                              <div className="px-3 py-2">
                                <div className="flex items-center justify-between">
                                  <p className="text-xs text-gray-500 dark:text-zinc-400 truncate">{pageName.toLowerCase().replace(/ /g, "")}.com</p>
                              ...
                              <div className="flex items-center justify-between px-3 py-1.5 border-t border-zinc-100">
                                <div className="flex items-center gap-3 text-gray-400 dark:text-zinc-600">
                                  <span>â™¡</span><span>ðŸ’¬</span><span>âŠ˜</span>
                                </div>
                                <span className="text-gray-400 dark:text-zinc-600">ðŸ”–</span>
                              </div>
                          ...
                          <div className="bg-white rounded-xl overflow-hidden shadow-lg w-64">
                            ...
                            <div className="px-3 pt-2 pb-1">
                              <div className="flex items-center justify-between mb-1.5">
                                <div className="flex items-center gap-3 text-gray-400 dark:text-zinc-700">
                                  <span className="text-lg">â™¡</span>
                                  <span className="text-lg">ðŸ’¬</span>
                                  <span className="text-lg">â†—</span>
                                </div>
                                <span className="text-lg text-gray-400 dark:text-zinc-700">ðŸ”–</span>
                              </div>
                              {(ad.headline || ad.copy) && (
                                <div className="flex items-center justify-between mt-1 border-t border-zinc-100 pt-1.5">
                                  <div className="flex-1 min-w-0">
                                    {ad.website_url && <p className="text-[9px] text-gray-500 dark:text-zinc-400 uppercase truncate">{ad.website_url.replace(/https?:\/\//, "").split("/")[0]}</p>}
                        ...
                        <div className={`bg-white rounded-xl overflow-hidden shadow-lg ${previewTab === "mobile" ? "w-64" : "w-80"}`}>
                          <div className="p-3">
                            <div className="flex items-center justify-between mb-2">
                              <div className="flex items-center gap-2">
                                <div className="w-8 h-8 rounded-full bg-zinc-300 flex items-center justify-center text-xs font-bold text-gray-400 dark:text-zinc-700">{pageInitial}</div>
                                <div>
                                  <p className="text-xs font-bold text-zinc-900">{pageName}</p>
                                  <p className="text-[10px] text-gray-500 dark:text-zinc-500">Sponsored Â· ðŸŒ</p>
                                </div>
                              </div>
                              <span className="text-gray-500 dark:text-zinc-400 text-lg">Â·Â·Â·</span>
                            </div>
                            ...
                            {(ad.headline || ad.website_url) && (
                              <div className="mt-2 flex items-center justify-between">
                                <div className="flex-1 min-w-0">
                                  {ad.website_url && <p className="text-[10px] text-gray-500 dark:text-zinc-400 uppercase truncate">{ad.website_url.replace(/https?:\/\//, "").split("/")[0]}</p>}
                            ...
                            <div className="flex items-center gap-4 mt-3 pt-2 border-t border-zinc-100">
                              {["ðŸ‘ Like", "ðŸ’¬ Comment", "â†— Share"].map(label => (
                                <button key={label} className="flex-1 text-center text-[11px] text-gray-500 dark:text-zinc-500 font-semibold">{label}</button>
                              ))}
                            </div>
                          </div>
                        </div>
                    ...
                    <p className="text-[10px] text-gray-400 dark:text-zinc-700 text-center px-4 max-w-xs">
            ...
            {step === 4 && (
              <div className="p-6 space-y-6">
                <div>
                  <h3 className="text-base font-bold text-gray-900 dark:text-white mb-1">Review and publish</h3>
                  <p className="text-sm text-gray-500 dark:text-zinc-500">Review your campaign settings before publishing to Meta.</p>
                </div>
                <div className="space-y-4">
                  {[
                    { label: "Campaign name",      value: campName },
                    ...
                    <div key={label} className="flex items-start gap-4 py-3 border-b border-gray-200 dark:border-zinc-800/50">
                      <p className="text-xs text-gray-500 dark:text-zinc-500 w-36 shrink-0 pt-0.5">{label}</p>
        ...
        {/* Footer */}
        <div className="shrink-0 flex items-center justify-between px-6 py-4 border-t border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-900">
          <div className="flex items-center gap-3">
            <button onClick={onClose} className="px-4 py-2 text-sm text-gray-500 dark:text-zinc-400 hover:text-white transition-colors">Cancel</button>
            {step > 1 && (
              <button onClick={() => { setStep(s => s - 1); setFieldErrors({}); }}
                className="flex items-center gap-1.5 px-4 py-2 text-sm text-gray-500 dark:text-zinc-400 hover:text-white transition-colors">
          ...
          <div className="flex items-center gap-3">
            <button onClick={saveAsDraft} disabled={saving}
              className="px-4 py-2 bg-gray-200 dark:bg-zinc-800 hover:bg-gray-300 dark:bg-zinc-700 text-gray-900 dark:text-white text-sm font-semibold rounded-lg transition-colors disabled:opacity-50">
