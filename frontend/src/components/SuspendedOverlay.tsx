...
import { PauseCircle, Trash2, LogOut, CreditCard, Sparkles, Ban } from "lucide-react";
...
interface Props {
  orgName?: string;
  type: 'paused' | 'deleted' | 'restricted';
    ...
    return (
      <div className="flex items-center justify-center min-h-full p-6">
        <div className="w-full max-w-[440px] bg-[var(--card)] border border-[var(--border)] rounded-3xl p-12 text-center shadow-2xl shadow-black/30 animate-in fade-in zoom-in duration-500">
          <div className="w-16 h-16 rounded-2xl bg-rose-500/10 text-rose-400 shadow-lg shadow-rose-500/10 flex items-center justify-center mx-auto mb-8">
          ...
          <p className="text-[var(--foreground-muted)] text-sm leading-relaxed mb-8">
            Your organization{orgName && <span className="text-[var(--foreground)] font-semibold"> {orgName}</span>} has been
            temporarily banned by the administrator. You no longer have access to ZoikoVertex features.
          </p>

          <p className="text-[var(--foreground-muted)] text-xs leading-relaxed mb-8">
            This is a temporary ban. Contact the administrator to restore access.
          </p>
          ...
          <p className="text-[var(--foreground-muted)] text-sm leading-relaxed mb-10">
            ...
            page to contact the administrator for assistance.
          </p>
  ...
  return (
    <div className="flex items-center justify-center min-h-full p-6">
      <div className="w-full max-w-[440px] bg-[var(--card)] border border-[var(--border)] rounded-3xl p-12 text-center shadow-2xl shadow-black/30 animate-in fade-in zoom-in duration-500">
        ...
        <h2 className="text-2xl font-bold text-[var(--foreground)] mb-4 tracking-tight">
          Organization {isPaused ? 'Paused' : 'Permanently Deleted'}
        </h2>

        <p className="text-[var(--foreground-muted)] text-sm leading-relaxed mb-8">
          Your organization{" "}
          {orgName && <span className="text-[var(--foreground)] font-semibold">{orgName}</span>}{" "}
          has been {isPaused ? 'paused' : 'permanently deleted'}. You no longer have access to ZoikoVertex features.
        </p>

        {isPaused ? null : (
          <p className="text-[var(--foreground-muted)] text-xs leading-relaxed mb-8">
            This is a permanent ban. This action cannot be undone.
          </p>
        )}
