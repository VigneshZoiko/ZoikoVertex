...
const isRenderableSrc = (src?: string | null): src is string =>
      ...
      >
        ...
        <div className="flex-1 min-w-0 flex items-center gap-1.5 flex-wrap">
          ...
          {selectedInPlatform.map(acc => (
            ...
            >
              {isRenderableSrc(acc.avatar_url) && (
                <span className="w-3 h-3 rounded-full overflow-hidden inline-block shrink-0 relative flex items-center justify-center">
                  {brokenAvatars[acc.id]
                    ? <span className="flex items-center justify-center" style={{ color: meta.color }}><Icon className="w-2.5 h-2.5" /></span>
                    : <Image src={acc.avatar_url} alt="" fill className="object-cover" onError={() => setBrokenAvatars((prev) => ({ ...prev, [acc.id]: true }))} />}
                </span>
              )}
                    ...
                    <div className="w-7 h-7 rounded-full overflow-hidden border border-[var(--border)] shrink-0 relative bg-[var(--surface)] flex items-center justify-center">
                      {isRenderableSrc(account.avatar_url) && !brokenAvatars[account.id]
                        ? <Image
                          ...
                          />
                        : <span className="flex items-center justify-center" style={{ color: meta.color }}><Icon className="w-3.5 h-3.5" /></span>
                      }
                    </div>
